...
## Jev AI-assisted design and app control

**Ask Jev** in the designer/source headers opens a dockable prompt panel; **Jev · app** opens global app control. TypeSafe's native typed decisions become reviewed AST edits, exact literal changes, registered-control insertion, starter documents and allowed existing app commands. Context is scoped and byte-bounded; proposals do not modify the document until Apply and stale source/selection is rejected. An explicitly configured **separate** text-generating backend can create bespoke XAML/HTML, edit complete selected targets and repair invalid drafts, with Jev verification and normal history. Jev itself does not generate arbitrary source.

Settings include endpoint, model/pin and discovery, personal key/private proxy token, memory or opt-in tab-session credentials, request/context/confidence limits, and optional generator configuration. Use `npm run start:ai:pages` plus **Jev Settings → Use local bridge** to connect from the published GitHub Pages Studio without moving its workspace; direct TypeSafe access can be blocked by provider CORS. The command prints a private access token and accepts your own TypeSafe key only when explicitly configured. For locally served Studio with server-held keys, use `npm run start:ai`. See [setup, prompts, API contracts, security and qualification boundaries](docs/JEV-INTEGRATION.md). The reusable `@wieslawsoltes/xamora-assistant` package provides the same protocol and planner without Studio. Provider calls require explicit consent and can be billed; automated tests use mocked responses, not a paid semantic-quality qualification.
...
- Code highlighting, line numbers, XML validation, formatting, tag/property completion, find/replace, indentation, comment toggling, document symbols, selected-element navigation, and automatic bidirectional synchronization. [Structural source tooling](docs/STRUCTURAL-LANGUAGE-TOOLS.md) adds matching-tag navigation, nested syntax selection, and source-to-designer selection. [Namespace-aware completion](docs/NAMESPACE-COMPLETION.md) adds scoped XAML aliases, inline/owner property contexts, and HTML/SVG/MathML tag and attribute tooling.
...
[Code Editor Lab](https://wieslawsoltes.github.io/XamoraStudio/examples/EditorLab/),
[Property Grid Lab](https://wieslawsoltes.github.io/XamoraStudio/examples/PropertyGridLab/) and
[Dialog Lab](https://wieslawsoltes.github.io/XamoraStudio/examples/DialogLab/).

`@wieslawsoltes/xamora-docking`, `xamora-control-primitives`, `xamora-code-editor` and
`xamora-property-grid` / `xamora-dialogs` use the same canonical modules as Studio, with separate ESM/CommonJS
