# @wieslawsoltes/xamora-motion-workspace

Reusable document-aware components: **BlendFeatures, AnimationEditor, TimelineWorkspace, MotionRuntime**. Canonical source lives in `dist/workspaces/`; the package does not import Studio or start an application when imported.

```js
import { BlendFeatures } from '@wieslawsoltes/xamora-motion-workspace';
