# @wieslawsoltes/xamora-canvas-workspace

Reusable document-aware components: **CanvasController**. Canonical source lives in `dist/workspaces/`; the package does not import Studio or start an application when imported.

```js
import { CanvasController } from '@wieslawsoltes/xamora-canvas-workspace';
import '@wieslawsoltes/xamora-canvas-workspace/canvas-workspace.css';
```
