...
export class BlendFeatures {
  constructor(studio) {
    const s = (this.s = studio);
    s.blend = this;
    this.animation = new AnimationEditor(s);
    this.previewRuntime = null;
    this.previewFrame = 0;
    const command = s.command.bind(s);
    s.command = (action, event) => {
    ...
    s.renderer.resourceResolver = (source) =>
      s.stores
    ...
    $('.toolbar').insertBefore(toolbar, $('.toolbar .view-toggle'));
    Object.assign(window.xamora, { animation: this.animation, blend: this });
    s.render();
  }
  get commands() {
    return {
  ...
  async openExample() {
    ...
    try {
      const response = await fetch('./examples/MotionLab.xaml');
      if (!response.ok) throw Error('Motion example could not be loaded.');
      const { parseXaml } = await import('../core/xaml.js'),
        doc = parseXaml(await response.text(), { name: 'MotionLab.xaml', framework: 'WPF' });
  ...
  brush() {
    ...
    const show = () => {
      ...
      const capture = () => {
        draft.opacity = $('[name=brush-opacity]').value;
        for (const [key, name] of Object.entries({
          ...
          if ($(`[name=${name}]`)) draft[key] = $(`[name=${name}]`).value;
        if (kind !== 'Solid')
          draft.stops = $$('[data-stop]').map((row) => ({
            color: $('[data-stop-color]', row).value,
            offset: $('[data-stop-offset]', row).value,
          }));
      };
      $('[name=brush-property]').onchange = (e) => {
        property = e.target.value;
        load();
        show();
      };
      $('[name=brush-kind]').onchange = (e) => {
        capture();
        kind = e.target.value;
        show();
      };
      $('#brush-add-stop')?.addEventListener('click', () => {
        capture();
        draft.stops.push({ color: '#FFFFFF', offset: '0.5' });
        show();
      });
      $$('[data-stop-remove]').forEach(
        (button) =>
          (button.onclick = () => {
            capture();
        ...
        },
        {
          ...
          run: () => {
            ...
            for (const [key, title] of fields)
              next[key] = number($('[name=transform-' + key + ']').value, title);
            const origin = $('[name=transform-origin]').value;
      ...
      [
        {
          ...
          primary: true,
          run: () => {
            const kind = $('[name=effect-kind]').value,
              blur = number($('[name=effect-blur]').value, 'Blur', { min: 0 }),
              depth = number($('[name=effect-depth]').value, 'Shadow depth'),
              direction = number($('[name=effect-direction]').value, 'Direction'),
              opacity = number($('[name=effect-opacity]').value, 'Opacity', { min: 0, max: 1 }),
              color = $('[name=effect-color]').value,
              rect = $('[name=clip-rect]').value.trim(),
              radius = number($('[name=clip-radius]').value, 'Corner radius', { min: 0 });
            if (
              rect &&
              (rect.split(',').length !== 4 ||
  ...
  designValues() {
    ...
    const show = () => {
      ...
      const capture = () =>
        (rows = $$('[data-design-row]').map((row) => ({
          property: $('[data-design-key]', row).value.trim(),
          value: $('[data-design-value]', row).value,
        })));
      $('#design-value-add').onclick = () => {
        capture();
        rows.push({ property: '', value: '' });
        show();
      };
      $('#design-sample-items').onclick = () => {
        capture();
        rows = rows.filter((row) => row.property !== 'ItemsSource');
        rows.push({ property: 'ItemsSource', value: '{d:SampleData ItemCount=5}' });
        show();
      };
      $$('[data-design-remove]').forEach(
        (button) =>
          (button.onclick = () => {
            capture();
  ...
  resetLayout() {
    ...
    notify('Local layout values reset. Parent layout now determines placement.');
  }
  style() {
    this.wpf();
    ...
    const show = () => {
        ...
        [
          {
            ...
            run: () => {
              ...
              const clearLocals = $('[name=style-clear-locals]')?.checked;
      ...
      if (keyed) $('[name=style-key]').readOnly = true;
      const capture = () => {
        key = $('[name=style-key]').value.trim();
        targetType = $('[name=style-target]').value.trim();
        basedOn = $('[name=style-based]').value.trim();
        const readRows = (holder) =>
          $$(':scope > [data-setter-row]', holder).map((row) => ({
            property: $('[data-setter-property]', row).value.trim(),
            value: $('[data-setter-value]', row).value,
            target: $('[data-setter-target]', row).value.trim(),
          }));
        setters = readRows($('#style-setters'));
        triggers = $$('[data-style-trigger]').map((row) => ({
          property: $('[data-trigger-property]', row).value.trim(),
          value: $('[data-trigger-value]', row).value,
          setters: readRows($('[data-trigger-setters]', row)),
        }));
      };
      $('#style-add-setter').onclick = () => {
        capture();
        setters.push({ property: 'Foreground', value: '#7953E8', target: '' });
        show();
      };
      $('#style-add-trigger').onclick = () => {
        capture();
        triggers.push({
      ...
      $$('[data-trigger-setter-add]').forEach(
        (button) =>
          (button.onclick = () => {
            capture();
  ...
  states() {
    ...
    $$('[data-state-group-delete]').forEach(
      (button) =>
        (button.onclick = () => {
    ...
    $$('[data-state-value]').forEach(
      (button) => (button.onclick = () => this.stateValue(button.dataset.stateValue)),
    );
    $$('[data-state-preview]').forEach(
      (button) =>
        (button.onclick = () => {
          this.previewState(button.dataset.stateGroup, button.dataset.statePreview);
          $$('[data-state-preview]').forEach((b) => b.classList.toggle('active', b === button));
        }),
    );
    $$('[data-transition-new]').forEach(
      (button) => (button.onclick = () => this.transition(button.dataset.transitionNew)),
    );
    $$('[data-transition-edit]').forEach(
      (button) =>
        (button.onclick = () =>
    ...
    this.s.modal(title, field('motion-object-name', 'Name', suggestion), [
      {
        label: 'Create',
        primary: true,
        run: () => save(identifier($('[name=motion-object-name]').value.trim())),
      },
    ]);
  }
  stateValue(stateId) {
      ...
      [
        {
          ...
          primary: true,
          run: () => {
            const id = $('[name=state-target]').value,
              property = $('[name=state-property]').value.trim(),
              value = $('[name=state-value]').value;
    ...
    s.modal(
      ...
      [
          ...
          : []),
        {
          ...
          primary: true,
          run: () => {
            const from = $('[name=transition-from]').value,
              to = $('[name=transition-to]').value,
              duration = number($('[name=transition-duration]').value, 'Duration', { min: 0 });
  ...
  previewState(groupId, stateId) {
    ...
    if (!this.previewRuntime) {
      ...
      this.previewStarted = performance.now() / 1000;
    }
  ...
  eventTrigger() {
      ...
      [
        {
          ...
          primary: true,
          run: () => {
            const event = $('[name=event-name]').value,
              type = $('[name=event-action]').value,
              key = $('[name=event-story]').value,
              begin = $('[name=event-begin]').value,
              offset = $('[name=event-offset]').value;
    ...
    $$('[data-event-delete]').forEach(
      (button) =>
        (button.onclick = () => {
          this.mutate('Delete event trigger', (doc) =>
            remove(doc.root, button.dataset.eventDelete),
          );
          this.eventTrigger();
        }),
    );
  ...
  path() {
      ...
      [
        {
          ...
          primary: true,
          run: () => {
            const data = serializePath(parsePath($('#vector-path-data').value));
    ...
    const render = () => {
      ...
      $('#vector-path-data').value = serializePath(commands);
      $$('[data-path-point]', svg).forEach(
        (handle) =>
    ...
    $$('[data-path-tool]').forEach(
      (button) =>
        (button.onclick = () => {
          tool = button.dataset.pathTool;
          $$('[data-path-tool]').forEach((b) => b.classList.toggle('active', b === button));
        }),
    );
    $('#path-close').onclick = () => {
    ...
    $('#path-delete').onclick = () => {
      ...
      if (commands.length && commands[0].type !== 'M') {
        notify('A path must begin with a move command.');
        commands = [];
      }
      selectedPoint = null;
      render();
    };
    $('#vector-path-data').onchange = () => {
      try {
        commands = parsePath($('#vector-path-data').value);
        render();
      } catch (error) {
        notify(error.message);
      }
