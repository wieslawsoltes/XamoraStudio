...
export class TimelineWorkspace {
  constructor(s) {
    ...
    a.render = () => {
      const scroll = $('.motion-track-area')?.scrollTop || 0,
        x = $('.motion-track-area')?.scrollLeft || 0;
      render();
      if (a.open) {
        this.render();
        const area = $('.motion-track-area');
        area.scrollTop = scroll;
        area.scrollLeft = x;
      }
    };
    a.dragKey = (e, b) => this.dragKey(e, b);
    a.newStoryboard = () => this.newStory();
  }
  get frames() {
  ...
  copy() {
    ...
    notify(this.clipboard.length + ' keyframes copied');
  }
  paste() {
  ...
  add(property = $('#motion-quick-property')?.value || 'Opacity') {
    const a = this.a,
      s = this.s;
    if (!s.selected.length) {
      notify('Select a control on the canvas.');
      return;
    }
  ...
  render() {
    ...
    panel.insertBefore(bar, $('.motion-track-area', panel));
    $('#motion-quick-add').onclick = () => this.add();
    $('#motion-fps').onchange = (e) => (this.fps = Number(e.target.value));
    $('#motion-snap').onchange = (e) => (this.snap = e.target.checked);
    $('#motion-timeline-zoom').oninput = (e) => {
      this.zoom = Number(e.target.value);
      this.applyZoom();
    };
    $('#motion-inline-name').onchange = (e) => {
    ...
    $('#motion-inline-duration').onchange = (e) => {
    ...
    $('[data-key-time]', inspector).onchange = (e) =>
      this.shift(Number(e.target.value) - first.time);
    const edit = (options) =>
      this.changeKeys('Edit keyframes', (doc, ids) => {
        for (const id of ids) editKeyframe(doc, a.storyId, id, options);
        return ids;
      });
    $('[data-key-value]', inspector).onchange = (e) => edit({ value: e.target.value });
    $('[data-key-mode]', inspector).onchange = (e) =>
      edit({ interpolation: e.target.value, easing: $('[data-key-ease]', inspector).value });
    $('[data-key-ease]', inspector).onchange = (e) =>
      edit({ interpolation: 'Easing', easing: e.target.value });
    $('[data-key-remove]', inspector).onclick = () => this.remove();
    $('[data-key-duplicate]', inspector).onclick = () =>
      this.shift(
        Math.max(
          1 / this.fps,
    ...
    $('[data-key-advanced]', inspector).onclick = () => {
      a.trackId = first.trackId;
      a.keyDialog(first.id);
    };
    $$('[data-key-id]', panel).forEach((b) =>
      b.classList.toggle('selected', this.selected.has(b.dataset.keyId)),
    );
    panel.onkeydown = (e) => {
  ...
  applyZoom() {
    const area = $('.motion-track-area');
    if (!area) return;
    $$('.motion-ruler,.motion-track', area).forEach(
      (row) =>
