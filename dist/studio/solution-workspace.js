...
export class SolutionWorkspace {
  constructor(s) {
    ...
    s.renderer.resourceResolver = this.resolve;
    let saved;
    try {
      saved = JSON.parse(localStorage.getItem('xamora-workspace-v1'))?.solution;
    } catch {}
    ...
    s.docking.originalSwitch = (index) => {
      const old = s.doc.id;
  ...
  undo() {
    ...
    if (
      ...
      notify('Undo later document edits before undoing this solution change.');
      return;
    }
    this.history.pop();
  ...
  redo() {
    ...
    if (
      ...
      notify('The solution changed after undo.');
      return;
    }
    this.future.pop();
  ...
  rename(path) {
    ...
    if (!from) {
      notify('Select a file or folder.');
      return;
    }
  ...
  duplicate() {
    if (this.folder && !this.selectedId) {
      notify('Select a file to duplicate.');
      return;
    }
    const doc =
  ...
  remove() {
    if (this.folder && !this.selectedId) {
      notify('Select a file to remove.');
      return;
    }
    ...
    input.onchange = async () => {
      try {
        ...
        for (const file of input.files) {
          ...
          const doc = parseXaml(await file.text(), { name: file.name });
  ...
  save() {
    if (this.s.editor.composing) {
      notify('Finish composing the source text before saving the solution.');
      return;
    }
    this.s.sync?.flush();
    this.s.save();
  ...
  render() {
    ...
    this.rendering = true;
    const focused = this.host.contains(document.activeElement),
      focusKey =
        document.activeElement?.dataset.solutionRename !== undefined
          ? 'rename'
          : document.activeElement?.dataset.solutionSearch !== undefined
            ? 'search'
            : null,
      selection = focused && document.activeElement?.selectionStart;
    ...
    try {
      ...
      this.host.querySelector('[data-solution-search]').oninput = (e) => {
        this.query = e.target.value;
        this.render();
      };
      this.host.querySelector('[data-solution-name]').onchange = (e) =>
        this.mutate('Rename solution', (snap) => (snap.solution.name = e.target.value.trim()));
      this.host.querySelector('[data-solution-rename]').onchange = (e) => {
        try {
      ...
      this.host.querySelectorAll('[data-sol]').forEach(
        (b) =>
          (b.onclick = () => {
      ...
      $('.file-folder').textContent = this.model.name;
    } finally {
