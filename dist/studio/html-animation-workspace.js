...
export class HtmlAnimationWorkspace {
  constructor(html) {
    this.html = html;
    this.s = html.s;
    this.s.htmlAnimation = this;
    this.time = 0;
    this.rate = 1;
    this.loop = true;
    this.playing = false;
    ...
    css.rel = 'stylesheet';
    css.href = new URL('../styles/html-animation.css', import.meta.url).href;
    ...
    animation.render = () => (isHtml(this.s.doc) ? this.render() : render());
    animation.stop = () => {
    ...
    this.s.docking.visibility = (id, visible) => {
    ...
    const menu = (id, run, enabled = () => true, checked) => {
      ...
      item.run = (...args) => (isHtml(this.s.doc) ? run(...args) : originalRun(...args));
      item.enabled = () =>
        isHtml(this.s.doc)
          ? enabled()
  ...
  refresh() {
    ...
    const legacy = document.getElementById('animation-panel');
  ...
  async openExample() {
    ...
    try {
      const response = await fetch(new URL('../examples/HtmlMotionLab.html', import.meta.url));
      if (!response.ok) throw Error('Could not load the HTML motion example.');
  ...
  create(config) {
    ...
    if (!node || ['html', 'head', 'style', 'script', 'title', 'meta', 'link'].includes(node.type)) {
      notify('Select a visual HTML element to animate.');
      return;
    }
    const preset =
  ...
  toggleRecord() {
    this.pause();
    if (!this.target) {
      notify('Select the HTML element to record.');
      return;
    }
  ...
  recordMutation(doc, action) {
    ...
    walk(doc.root, (n) => {
      if (n.kind === 'element')
        before.set(n.id, { style: n.props.style, values: styles(n.props.style) });
    });
    action(doc);
    const changes = [];
    walk(doc.root, (n) => {
      ...
      const next = styles(n.props.style),
  ...
  play() {
    if (!this.catalog.bindings.length) {
      notify('Create or bind an HTML animation first.');
      return;
    }
    this.recording = false;
  ...
  transport() {
    ...
    if (time && document.activeElement !== time) time.value = (this.time / 1000).toFixed(3);
    const scrub = panel.querySelector('[data-hm-scrub]');
    if (scrub && document.activeElement !== scrub) scrub.value = String(this.time);
    panel
  ...
  <footer role="status" title="${esc(this.catalog.diagnostics.map((x) => x.message || x).join(' · '))}">${this.catalog.diagnostics.length ? esc(this.catalog.diagnostics.map((x) => x.message || x).join(' · ')) : this.recording ? 'Recording CSS changes at the playhead. Base styles remain unchanged.' : 'CSS @keyframes and animation properties are the source of truth. Playback samples never enter undo history.'}</footer>`;
    ...
    q('[data-hm-definition]').onchange = (e) => {
    ...
    q('[data-hm-play]').onclick = () => (this.playing ? this.pause() : this.play());
    q('[data-hm-stop]').onclick = () => this.stop();
    q('[data-hm-record]').onclick = () => this.toggleRecord();
    q('[data-hm-time]').onchange = (e) => {
      this.pause();
      this.seek(number(e.target.value) * 1000);
    };
    q('[data-hm-scrub]').oninput = (e) => {
      this.pause();
      this.seek(number(e.target.value));
    };
    q('[data-hm-loop]').onchange = (e) => (this.loop = e.target.checked);
    q('[data-hm-rate]').onchange = (e) => (this.rate = number(e.target.value, 1));
    q('[data-hm-snap]').onchange = (e) => (this.snap = e.target.checked);
    q('[data-hm-zoom]').oninput = (e) => {
      this.zoom = number(e.target.value, 1);
      this.zoomRows();
    };
    q('[data-hm-property]').onchange = (e) => (this.property = e.target.value);
    q('[data-hm-add-key]').onclick = () => this.addKey();
    this.host.querySelectorAll('[data-hm-timing]').forEach(
      (input) =>
        (input.onchange = () => {
    ...
    this.host.querySelectorAll('[data-hm-track]').forEach(
      (el) =>
        (el.onclick = () => {
    ...
    this.host.querySelectorAll('[data-hm-key]').forEach(
      (el) =>
        (el.onpointerdown = (e) => {
    ...
    q('[data-hm-ruler]').onpointerdown = (e) => this.scrubGesture(e, scrub);
    this.host.querySelectorAll('[data-hm-lane]').forEach(
      (el) =>
        (el.ondblclick = (e) => {
    ...
    this.host.querySelectorAll('[data-hm-key-value]').forEach(
      (el) =>
        (el.onchange = () =>
    ...
    this.host.querySelectorAll('[data-hm-remove-property]').forEach(
      (el) =>
        (el.onclick = () =>
    ...
    q('[data-hm-duplicate-key]').onclick = () => {
    ...
    this.host.onkeydown = (e) => this.key(e);
