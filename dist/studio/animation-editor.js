...
export class AnimationEditor {
  constructor(studio) {
    ...
    this.player = new AnimationPlayer({ duration: 3 });
    this.player.addEventListener('frame', () => this.frame());
    for (const event of ['play', 'pause', 'ended'])
      this.player.addEventListener(event, () => this.updateTransport());
  ...
  recordValues(ids, key, value) {
    ...
    if (ids.some((id) => isLocked(this.s.doc, id))) {
      notify('Unlock the layer before recording its properties.');
      return true;
    }
    ...
    if (String(value).startsWith('{')) {
      notify('Record a literal value. Bindings remain authored base values.');
      return true;
    }
    this.recordBatch(
  ...
  recordBatch(changes, title = 'Record canvas properties') {
    if (!this.story) {
      notify('Select a Storyboard before recording.');
      return false;
    }
    if (changes.some((c) => isLocked(this.s.doc, c.id))) {
      notify('Unlock the layer before recording.');
      return false;
    }
  ...
  render() {
    if (!this.open) return;
    let panel = $('#animation-panel');
    if (!panel) {
      panel = document.createElement('section');
      panel.id = 'animation-panel';
      panel.className = 'motion-panel';
      (this.s.docking?.timelineHost || $('.center')).append(panel);
    }
    ...
    $('#motion-new').onclick = () => this.newStoryboard();
    $('#motion-settings').onclick = () => this.settings();
    $('#motion-close').onclick = () => this.toggle();
    $('#motion-story').onchange = (e) => {
      this.stop();
      this.storyId = e.target.value;
      this.trackId = null;
      this.render();
    };
    $('#motion-record').onclick = () => {
    ...
    $('#motion-stop').onclick = () => this.stop();
    $('#motion-time').onchange = (e) => this.seek(safeNumber(e.target.value));
    $('#motion-loop').onchange = (e) => (this.player.loop = e.target.checked);
    $('#motion-rate').onchange = (e) => (this.player.rate = Number(e.target.value));
    $('#motion-add-track').onclick = () => this.addTrackDialog();
    $('#motion-add-key').onclick = () => this.keyDialog();
    const ruler = $('#motion-ruler-track');
    ...
    $$('[data-track-select]', panel).forEach(
      (button) =>
        (button.onclick = () => {
    ...
    $$('[data-key-id]', panel).forEach((button) => {
      button.onpointerdown = (e) => this.dragKey(e, button);
      button.ondblclick = (e) => {
        e.stopPropagation();
  ...
  updateTransport() {
    ...
    if (time && document.activeElement !== time) time.value = Number(this.player.time.toFixed(3));
    $$('.motion-playhead', panel).forEach(
    ...
    $('#motion-ruler-track')?.setAttribute('aria-valuenow', String(this.player.time));
  }
  newStoryboard() {
    if (this.s.doc.framework !== 'WPF') {
      notify('Native Storyboard authoring is currently available for WPF documents.');
      return;
    }
    this.s.modal(
      'New Storyboard',
      ...
      [
        {
          ...
          primary: true,
          run: () => {
            const name = $('[name=motion-name]').value.trim(),
              duration = Number($('[name=motion-duration]').value);
      ...
      [
        ...
        },
        {
          ...
          run: () => {
            const name = $('[name=motion-name]').value.trim(),
              duration = Number($('[name=motion-duration]').value),
              speed = Number($('[name=motion-speed]').value),
              begin = $('[name=motion-begin]').value,
              repeat = $('[name=motion-repeat]').value;
            if (
            ...
            const values = {
              ...
              FillBehavior: $('[name=motion-fill]').value,
              AutoReverse: $('[name=motion-reverse]').checked ? 'True' : 'False',
            };
            if (
      ...
      [
        {
          ...
          primary: true,
          run: () => {
            const targetId = $('[name=motion-target]').value,
              property =
                $('[name=motion-custom]').value.trim() || $('[name=motion-property]').value,
              id = this.storyId;
  ...
  keyDialog(keyId) {
    ...
    this.s.modal(
      ...
      [
        ...
        },
        {
          ...
          primary: true,
          run: () => {
            const time = Number($('[name=key-time]').value),
              value = $('[name=key-value]').value,
              interpolation = $('[name=key-interpolation]').value,
              spline = $('[name=key-spline]').value,
              easeName = $('[name=key-easing]').value,
              easeMode = $('[name=key-easing-mode]').value;
    ...
    const curve = () => {
      try {
        const interpolation = $('[name=key-interpolation]').value,
          spline = $('[name=key-spline]').value,
          easing = $('[name=key-easing]').value,
          mode = $('[name=key-easing-mode]').value;
    ...
    for (const name of ['key-interpolation', 'key-easing', 'key-easing-mode', 'key-spline'])
      $(`[name=${name}]`).addEventListener('change', curve);
    curve();
  }
