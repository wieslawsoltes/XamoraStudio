...
export class CanvasController {
  constructor(studio) {
    this.s = studio;
    this.hit = new HitTestService(studio);
    this.gesture = null;
    ...
    studio.pointerDown = (e) => this.pointerDown(e);
    studio.pointerMove = (e) => this.pointerMove(e);
    studio.pointerUp = (e, cancel) => this.pointerUp(e, cancel);
    const draw = studio.drawSelection.bind(studio);
    studio.drawSelection = () => {
      draw();
      this.drawGridOverlay();
    };
    const context = studio.contextMenu.bind(studio);
    studio.contextMenu = (x, y) => {
      context(x, y);
      ...
      $('[data-advanced-pick]', menu).onclick = () => {
        menu.remove();
        this.pickMenu(x, y);
      };
    };
    const drop = studio.drop.bind(studio);
    studio.drop = (e, target, canvas) => {
    ...
    $('#canvas-viewport').tabIndex = 0;
    $('#left-content').tabIndex = 0;
    $('#left-content').addEventListener('dragover', (e) => this.treeCue(e));
    ...
    $('#canvas-viewport').addEventListener('dragover', (e) => {
    ...
    $('#canvas-viewport').addEventListener('dragleave', (e) => {
  ...
  pointerDown(e) {
    ...
    if (e.altKey) {
      ...
      notify(`Selected ${label(node)} · Alt-click to cycle`);
      return;
    }
    if (e.shiftKey)
      s.store.select(
  ...
  pointerUp(e, cancel = false) {
    ...
    try {
      if (g.kind === 'track') this.s.store.commitSnapshot('Resize grid tracks', g.before);
        ...
        if (!g.plan?.allowed) {
          notify(g.plan?.reason || 'Drop cancelled.');
          return;
        }
  ...
  clearCue() {
    $('#drop-cue')?.remove();
    $$('.smart-guide').forEach((el) => el.remove());
    $$('.tree-drop-before,.tree-drop-after').forEach((el) =>
      el.classList.remove('tree-drop-before', 'tree-drop-after'),
    );
  }
  treeCue(e) {
  ...
  drawGridOverlay() {
    ...
    for (const [axis, sizes, gap] of [
      ...
      sizes.forEach((size, index) => {
        ...
        b.onclick = () => this.gridEditor(axis, index);
        host.append(b);
        position += size;
  ...
  gridEditor(axis = 'Row', active = 0) {
    ...
    if (n && isLocked(s.doc, n.id)) {
      notify('Unlock this grid before editing tracks.');
      return;
    }
    if (!n || localName(n.type) !== 'Grid') {
      notify('Select a Grid first.');
      return;
    }
    const nodeId = n.id;
    const render = () => {
        ...
        [
          {
            ...
            run: () => {
              ...
              $$('[data-track-index]').forEach((row) => {
                const d = next[Number(row.dataset.trackIndex)];
                $$('[data-track-prop]', row).forEach((input) => {
