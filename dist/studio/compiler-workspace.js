...
export class CompilerWorkspace {
  ...
  readSettings() {
    ...
    if (width || height) {
      if (
        ![width, height].every(
          (v) => v !== '' && Number.isFinite(Number(v)) && Number(v) > 0 && Number(v) <= 100000,
        )
      )
        throw Error(
