...
export class RichProperties {
  ...
  mutate(label, fn) {
    ...
    if (s.selected.some((n) => isLocked(s.doc, n.id))) {
      notify('Unlock the selection before editing.');
      return;
    }
    try {
  ...
  decorate() {
    ...
    $$('[data-prop]', host).forEach((input) => {
      ...
      if (numeric.has(key) || descriptor?.type === 'number') {
        ...
        label.onpointerdown = (e) => this.scrub(e, ids, key, input.value);
      }
      if (colors.has(key)) {
      ...
      if (
        ...
        [...input.options]
          ...
          .forEach((option) => {
            ...
            b.onclick = () => this.commit(ids, key, option.value);
            buttons.append(b);
          });
    ...
    this.brush($('.rich-brush-body', extras), node, ids);
    this.transforms($('.rich-transforms', extras), node, ids);
    this.effects($('.rich-effects', extras), node, ids);
  ...
  brush(host, node, ids) {
    ...
    $('.rich-ramp', host).style.background = brush
      ? brushCSS(brush)
      : colorParts(node.props[property])?.rgb || 'transparent';
    $('[data-brush-prop]', host).onchange = (e) => {
      this.brushProperty = e.target.value;
      this.s.renderInspector();
    };
    $('[data-brush-resource]', host).onclick = () => this.s.docking.control.show('assets');
    $('[data-brush-kind]', host).onchange = (e) =>
    ...
    $('[data-brush-color]', host)?.addEventListener('change', (e) => {
    ...
    $('[data-brush-opacity]', host).onchange = (e) =>
      edit('Brush opacity', (b) => (b.props.Opacity = e.target.value));
    $$('[data-brush-point]', host).forEach(
      (input) =>
        (input.onchange = () => {
          if (!/^\s*-?\d*\.?\d+\s*,\s*-?\d*\.?\d+\s*$/.test(input.value)) {
            notify('Enter two coordinates, such as 0,1.');
            return;
          }
  ...
  transforms(host, node, ids) {
    ...
    $$('[data-origin]', host).forEach(
      (b) => (b.onclick = () => this.commit(ids, 'RenderTransformOrigin', b.dataset.origin)),
    );
    $('[data-transform-reset]', host).onclick = () =>
  ...
  effects(host, node, ids) {
    ...
    $('[data-effect-type]', host).onchange = (e) =>
  ...
  style(host, node) {
    ...
    host.append(section);
    $$('[data-setter-property],[data-setter-value]', section).forEach(
      (input) =>
        (input.onchange = () =>
    ...
    $$('[data-setter-delete]', section).forEach(
      (b) =>
        (b.onclick = () =>
    ...
    $('[data-setter-add]', section).onclick = () =>
  ...
  grid(host, node) {
    ...
    $$('[data-grid-delete]', section).forEach(
      (b) =>
        (b.onclick = () =>
