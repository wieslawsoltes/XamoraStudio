...
export class ResourceWorkspace {
  ...
  render() {
    ...
    $('#resource-scope').value = this.scope;
    $('#resource-scope').onchange = (e) => {
      this.scope = e.target.value;
      this.render();
    };
    $('#resource-search').oninput = (e) => {
      this.query = e.target.value;
      this.render();
    };
    $$('[data-new-resource]', host).forEach(
      (b) => (b.onclick = () => this.create(b.dataset.newResource)),
    );
    $$('[data-resource]', host).forEach(
      (b) =>
        (b.onclick = () => {
    ...
    $('#merge-add').onclick = () => {
      ...
      try {
        s.store.transaction('Merge dictionary', (doc) =>
          this.merge(
            doc,
            dicts.find((d) => d.id === $('#merge-dictionary').value),
          ),
        );
      } catch (e) {
        notify(e.message);
    ...
    $('#merged-list').innerHTML = merged
      ...
      .join('');
    $$('[data-merge-remove],[data-merge-up]', host).forEach(
      (b) =>
        (b.onclick = () => {
          if (!s.prepareEdit()) return;
          s.store.transaction('Edit merged dictionaries', (doc) => {
            ...
            if (b.dataset.mergeRemove) p.children.splice(index, 1);
              ...
              [p.children[index], p.children[index - 1]] = [
                p.children[index - 1],
                p.children[index],
              ];
          });
    ...
    if (entry) this.details($('#resource-detail'), entry);
    host.scrollTop = scroll;
    if (wasSearch) {
      $('#resource-search').focus();
      if (Number.isInteger(caret)) $('#resource-search').setSelectionRange(caret, caret);
    }
  }
  details(host, entry) {
    const s = this.s,
    ...
    $('#resource-key').onchange = (e) => s.solution.renameKey(doc.id, node.id, e.target.value);
    ...
    $('#resource-apply').onclick = () => this.apply(entry, $('#resource-apply-property').value);
    $('#resource-edit').onclick = () => {
    ...
    $$('[data-resource-reference]', host).forEach(
      (b) =>
        (b.onclick = () => {
