import { parseHtml, serializeHtml, completeHtml, formatHtml } from './html.js';
...
export class XamlEditor {
  constructor(host, { registry, onApply, onSelection }) {
    ...
    this.input.addEventListener('keyup', () => this.cursor(true));
    this.input.addEventListener('keydown', (e) => this.keydown(e));
    host.querySelector('.editor-find').addEventListener('click', (e) => {
  ...
  setValue(value, { force = false, preserveHistory = false } = {}) {
    ...
    this.message.textContent = (this.language || 'XAML') + ' · synchronized';
    return true;
  }
  ...
  paint() {
    ...
    this.highlight.innerHTML =
      parts
        .map((t) =>
          t.startsWith('<!--')
            ? `<span class="syntax-comment">${esc(t)}</span>`
            : t[0] === '"' || t[0] === "'"
              ? `<span class="syntax-string">${esc(t)}</span>`
              : t[0] === '<' || t === '>' || t === '/>'
                ? `<span class="syntax-tag">${esc(t)}</span>`
                : /^[\w:.-]+$/.test(t)
                  ? `<span class="syntax-attr">${esc(t)}</span>`
                  : esc(t),
        )
        .join('') + '\n';
  ...
  validate() {
    ...
    try {
      (this.language === 'HTML' ? parseHtml : parseXaml)(this.input.value);
      this.message.textContent = this.dirty
        ? 'Valid ' + (this.language || 'XAML') + ' · synchronizing…'
        : (this.language || 'XAML') + ' · synchronized';
  ...
  format() {
    try {
      this.input.value =
        this.language === 'HTML'
          ? formatHtml(this.input.value)
          : serializeXaml(parseXaml(this.input.value));
      this.changed();
    } catch (e) {
      this.validate();
    }
  ...
  complete() {
    ...
    const items = (
      ...
      (this.language === 'HTML'
        ? completeHtml(this.input.value, this.input.selectionStart)
        : completeXaml(this.input.value, this.input.selectionStart, {
    ...
    for (const [index, entry] of items.entries()) {
      ...
      item.onmousedown = (e) => {
        e.preventDefault();
        if (this.input.value !== source || this.input.selectionStart !== caret) {
          this.hideCompletions();
          return;
        }
  ...
  keydown(e) {
    ...
    if (!this.completions.hidden && ['ArrowDown', 'ArrowUp', 'Enter', 'Tab'].includes(e.key)) {
      ...
      if (e.key === 'Enter' || e.key === 'Tab') {
        items[this.completionIndex].dispatchEvent(new MouseEvent('mousedown', { bubbles: true }));
        return;
      }
    ...
    if (e.key === 'Enter') {
      ...
      const a = this.input.selectionStart,
        ...
        indent = before.split('\n').at(-1).match(/^\s*/)[0];
      const extra = /<[^/!?][^>]*>$/.test(before) && !before.endsWith('/>') ? '    ' : '';
    ...
    if (mod && e.key === '/') {
      ...
      const out = text.trim().startsWith('<!--')
