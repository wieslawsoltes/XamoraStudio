...
export class DocumentSession extends EventTarget {
 constructor(store,{source,adapters={},incremental=true}={}){
  ...
  this.incremental=incremental;this.processingStats={fullParses:0,localParses:0,fullScans:0,localScans:0,charactersParsed:0,charactersScanned:0,incrementalUpdates:0,fullUpdates:0};this.lastUpdate={mode:'initial',reason:'session-created'};
 ...
 _parse(source){if(typeof source!=='string'||source.length>2_000_000)throw Error('Source must be text smaller than 2 MB.');this.processingStats.fullScans++;this.processingStats.charactersScanned+=source.length;const syntax=scanSource(source,{html:this.language==='HTML'});this.processingStats.fullParses++;this.processingStats.charactersParsed+=source.length;const doc=this.adapter.parse(source,{name:this.store.document.name,framework:this.store.document.framework});validateDocument(doc);return {doc,syntax};}
 _parseLocal(fragment){this.processingStats.localScans++;this.processingStats.charactersScanned+=fragment.length;scanSource(fragment,{html:this.language==='HTML'});this.processingStats.localParses++;this.processingStats.charactersParsed+=fragment.length;const doc=this.adapter.parse(fragment,{name:this.store.document.name,framework:this.store.document.framework});validateDocument(doc);return doc;}
 _sourcePreparation(text,base=this.store.document,knownEdit){
  const start={...this.processingStats};this._fallbackReason='structural-edit';let result;
  try{result=this._tryIncremental(text,base,knownEdit);if(!result)result={...this._parse(text),mode:'full',reason:this._fallbackReason};return result;}
  finally{const mode=result?.mode||this._localErrorMode||'full';this.lastUpdate={mode,reason:result?.reason||this._fallbackReason,input:knownEdit?'range':'snapshot',charactersParsed:this.processingStats.charactersParsed-start.charactersParsed,charactersScanned:this.processingStats.charactersScanned-start.charactersScanned,range:result?.edit?{start:result.edit.start,oldEnd:result.edit.end,newEnd:result.edit.start+result.edit.text.length}:null};if(mode.startsWith('incremental'))this.processingStats.incrementalUpdates++;else this.processingStats.fullUpdates++;this._localErrorMode=null;}
 }
 _tryIncremental(text,base,knownEdit){
  if(!this.incremental){this._fallbackReason='incremental-disabled';return null;}
  const oldSource=base.metadata?.source?.validText;
  if(typeof oldSource!=='string'||this.index?.source!==oldSource||text.length>2_000_000||(base.metadata?.source?.diagnostics||[]).some(d=>d.severity==='error')){this._fallbackReason='unavailable-valid-base';return null;}
  if(this.adapter!==sourceAdapters[this.language]){this._fallbackReason='custom-adapter';return null;}
  const edit=knownEdit||computeSourceEdit(oldSource,text);if(!edit)return null;
  const modelNodes=base===this.store.document&&this._lastRoot===base.root?this._modelNodes:new Map(nodes(base).map(n=>[n.id,n]));
  const namespaceContext=node=>Object.entries(node?.scope||{}).filter(([prefix])=>prefix!=='xml').map(([prefix,value])=>' xmlns'+(prefix?':'+prefix:'')+'="'+escapeXML(value)+'"').join('');
  const htmlContextSafe=id=>{const raw=this.index.tokenById.get(id);if(!raw)return false;let parentId=this.index.parentIds.get(id);while(parentId&&this.index.byId.get(parentId)?.synthetic)parentId=this.index.parentIds.get(parentId);const expected=parentId?this.index.tokenById.get(parentId):this.index.syntax.root;return raw.parent===expected;};
  const spans=this.index.spans;
  for(const span of spans){if(span.synthetic||span.start>edit.start||span.end<edit.end)continue;const node=modelNodes.get(span.nodeId);if(!node)continue;
   for(const attribute of span.attrs||[]){if(!attribute.quote||edit.start<attribute.valueStart||edit.end>attribute.valueEnd)continue;
    if(attribute.name==='xmlns'||attribute.name.startsWith('xmlns:')||attribute.name.startsWith('xml:')){this._fallbackReason='namespace-or-inherited-context';return null;}
    if(this.language==='HTML'&&(node.namespaceURI&&node.namespaceURI!=='http://www.w3.org/1999/xhtml'||!htmlContextSafe(node.id))){this._fallbackReason='foreign-or-repaired-html';return null;}
    if(span.attrs.filter(a=>a.name===attribute.name).length!==1){this._fallbackReason='duplicate-html-attribute';return null;}
    const value=oldSource.slice(attribute.valueStart,edit.start)+edit.text+oldSource.slice(edit.end,attribute.valueEnd);
    if(value.includes(attribute.quote)||value.includes('<')||value.includes('\u0000')){this._fallbackReason='attribute-token-boundary';return null;}
    const raw=oldSource.slice(attribute.start,attribute.valueStart)+value+oldSource.slice(attribute.valueEnd,attribute.end);
    const fragment=this.language==='HTML'?'<html><head></head><body><div '+raw+'></div></body></html>':'<__xamora'+namespaceContext(node)+'><probe '+raw+' /></__xamora>';
    try{const parsed=this._parseLocal(fragment),probe=this.language==='HTML'?parsed.root.children.find(n=>n.type==='body')?.children.find(n=>n.type==='div'):parsed.root.children.find(n=>n.kind==='element');if(!probe||Object.keys(probe.props).length!==1||!Object.hasOwn(probe.props,attribute.name)){this._fallbackReason='attribute-projection-ambiguity';return null;}
     return {mode:'incremental-attribute',reason:'quoted-attribute-value',edit,nodeId:node.id,attributeName:attribute.name,value:probe.props[attribute.name]};
    }catch(error){this._localErrorMode='incremental-invalid';error.start=edit.start;Object.assign(error,this.index.position(edit.start));throw error;}
   }
  }
  for(const span of spans){if(span.kind!=='text'||span.start>edit.start||span.end<edit.end)continue;const node=modelNodes.get(span.nodeId),parent=modelNodes.get(this.index.parentIds.get(span.nodeId));if(!node||!parent)continue;
   const raw=oldSource.slice(span.start,edit.start)+edit.text+oldSource.slice(edit.end,span.end);
   if(!raw||raw.includes('<')||raw.includes('\u0000')){this._fallbackReason='text-token-boundary';return null;}
   if(this.language==='HTML'){
    const contexts=new Set([...HTML_RAW,'title','textarea','pre','listing','table','thead','tbody','tfoot','tr','colgroup','select','option','optgroup','template','svg','math','head','html']);let p=parent;while(p){if(contexts.has(p.type)&&!['html'].includes(p.type)||p.namespaceURI&&p.namespaceURI!=='http://www.w3.org/1999/xhtml'){this._fallbackReason='contextual-html-text';return null;}p=modelNodes.get(this.index.parentIds.get(p.id));}
    if(!htmlContextSafe(node.id)){this._fallbackReason='repaired-html-text';return null;}
   }
   const fragment=this.language==='HTML'?'<html><head></head><body><div>'+raw+'</div></body></html>':'<__xamora'+namespaceContext(parent)+(parent.space==='preserve'?' xml:space="preserve"':'')+'>'+raw+'</__xamora>';
   try{const parsed=this._parseLocal(fragment),probe=this.language==='HTML'?parsed.root.children.find(n=>n.type==='body')?.children.find(n=>n.type==='div'):parsed.root;if(probe?.children.length!==1||probe.children[0].kind!=='text'){this._fallbackReason='text-node-topology';return null;}return {mode:'incremental-text',reason:'ordinary-text-token',edit,nodeId:node.id,value:probe.children[0].text};}
   catch(error){this._localErrorMode='incremental-invalid';error.start=edit.start;Object.assign(error,this.index.position(edit.start));throw error;}
  }
  return null;
 }
 _applyLocal(doc,prepared){const node=find(doc.root,prepared.nodeId);if(!node)throw Error('The edited AST node no longer exists.');if(prepared.attributeName)Object.defineProperty(node.props,prepared.attributeName,{value:prepared.value,enumerable:true,configurable:true,writable:true});else node.text=prepared.value;}
 ...
 _beforeCommit({before,document:after}){
  ...
  let parsed=this._sourcePreparation(text,before);let expected=parsed.doc;if(parsed.mode.startsWith('incremental')){expected=clone(before);this._applyLocal(expected,parsed);if(documentShape(expected)!==documentShape(after)){parsed={...this._parse(text),mode:'full',reason:'combined-model-change'};expected=parsed.doc;this.processingStats.incrementalUpdates--;this.processingStats.fullUpdates++;this.lastUpdate={...this.lastUpdate,mode:'full',reason:parsed.reason,charactersParsed:this.lastUpdate.charactersParsed+text.length,charactersScanned:this.lastUpdate.charactersScanned+text.length};}}if(documentShape(expected)!==documentShape(after))throw Error('This visual edit would change additional markup during parsing. Make the affected source container explicit before editing it.');
  this._record(after,text,text);this._pendingParsed={source:text,...parsed,document:after};this.origin='visual';
 }
 updateSource(text,options={}){return this._updateSource(text,options);}
 _updateSource(text,{origin='code',expectedRevision}={},knownEdit){
  ...
  let parsed,errors=[];try{parsed=this._sourcePreparation(text,this.store.document,knownEdit);}catch(error){errors=[this._diagnostic(error,text)];}
  ...
  try{this.store.transaction(errors.length?'Edit source draft':'Edit '+this.language+' source',doc=>{
   if(parsed){const dimensions={Width:doc.root.props.Width,Height:doc.root.props.Height};if(parsed.mode.startsWith('incremental'))this._applyLocal(doc,parsed);else{reconcileDocumentIds(doc,parsed.doc);doc.root=parsed.doc.root;doc.preamble=parsed.doc.preamble||[];doc.postamble=parsed.doc.postamble||[];if(this.language==='HTML')doc.metadata.html={...doc.metadata.html,...parsed.doc.metadata.html};}if(this.language==='XAML')for(const [property,dimension] of [['Width','width'],['Height','height']]){const value=doc.root.props[property];if(value!==dimensions[property]&&value!==undefined&&String(value).trim()!==''&&Number.isFinite(Number(value))&&Number(value)>0)doc.design[dimension]=Number(value);}this._record(doc,text,text);this._pendingParsed={source:text,...parsed,document:doc};}
 ...
 applySourceEdits(edits,{origin='code',expectedRevision,expectedVersion}={}){
  if(expectedRevision!==undefined&&expectedRevision!==this.revision)return {accepted:false,valid:this.isValid,revision:this.revision,diagnostics:[{severity:'error',code:'revision-conflict',message:'The document changed since these source edits were created.'}]};
  const proposed=this.buffer.fork(),result=proposed.applyEdits(edits,{expectedVersion});if(!result.accepted)return {accepted:false,valid:this.isValid,revision:this.revision,diagnostics:[{severity:'error',code:result.reason||'invalid-source-edits',message:result.error||'The source edits are invalid.'}]};
  return this._updateSource(proposed.text,{origin,expectedRevision},result.edits.length===1?result.edits[0]:undefined);
 }
 ...
 refresh({emit=true}={}){
  ...
  if(parsed?.mode?.startsWith('incremental')){updateSourceIndex(this.index,parsed.edit,{nodeId:parsed.nodeId,attributeName:parsed.attributeName});this._lastShape=documentShape(doc);this._lastRoot=doc.root;this.buffer.applyEdits([parsed.edit]);if(emit)this._emit();return this;}
  ...
  reconcileDocumentIds(doc,parsed.doc);this.index=buildSourceIndex(source,parsed.doc,parsed.syntax);this._lastShape=documentShape(doc);this._lastRoot=doc.root;this._modelNodes=new Map(nodes(doc).map(n=>[n.id,n]));if(!this.buffer)this.buffer=new SourceTextBuffer(this.source);else this.buffer.replace(this.source);
