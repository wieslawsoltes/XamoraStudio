...
export function registerXamlElement(name = 'xamora-view', options = {}) {
  ...
  if (!registry || !BaseElement)
    throw new Error('Custom elements require a browser DOM. Register after the DOM is available.');
  if (!/^[a-z][a-z0-9._-]*-[a-z0-9._-]+$/.test(name))
    throw new Error('Use a lowercase custom element name containing a hyphen.');
  ...
  if (registry.get(name))
    throw new Error(`The custom element ${name} is already registered by another implementation.`);
  const {
    shadow = true,
  ...
  class XamlViewElement extends BaseElement {
    ...
    constructor() {
      ...
      this._data =
        defaultData instanceof ObservableState
          ? defaultData
          : new ObservableState(defaultData ?? {}).snapshot();
    ...
    get source() {
      return this._source ?? this._resolvedSource ?? defaultSource ?? '';
    }
    set source(value) {
    ...
    set data(value) {
      if (!value || typeof value !== 'object')
        throw new TypeError('data must be an object or ObservableState.');
      this._data = value;
      this._queueRefresh();
    }
      ...
      try {
        ...
        if (source === undefined) {
          ...
          if (src) {
            ...
            source =
              defaultSource ??
              this.querySelector('script[type="application/xaml"]')?.textContent ??
        ...
        if (!source.trim())
          throw new Error('Provide .source, a src attribute, or an application/xaml script.');
        ...
        this.dispatchEvent(
          new CustomEvent('xamora-error', { detail: { error }, bubbles: true, composed: true }),
        );
        throw error;
      } finally {
