...
export class SourceTextBuffer {
  ...
  constructor(text = '', {version = 0} = {}) {
    ...
    if (!Number.isSafeInteger(version) || version < 0) throw new TypeError('Version must be a nonnegative safe integer.');
    this.#text = text;
    this.#version = version;
    this.#lines = [0];
    this.#stats = {charsScanned: 0, updates: 0, fullResets: 1, diffCharsCompared: 0, offsetsShifted: 0};
  ...
  get lineStarts() { return [...this.#lines]; }
  ...
  applyEdits(edits, {expectedVersion} = {}) {
    ...
    for (const edit of edits) {
      if (!edit || !Number.isSafeInteger(edit.start) || !Number.isSafeInteger(edit.end) ||
          edit.start < 0 || edit.end < edit.start || edit.end > this.length || typeof edit.text !== 'string') {
        return this.#reject('invalid-edit', 'Each edit requires valid UTF-16 start/end offsets and string text.');
      }
    ...
    for (let i = 1; i < ordered.length; i++) {
      if (ordered[i].start < ordered[i - 1].end || ordered[i].start === ordered[i - 1].start) {
        return this.#reject('overlapping-edits', 'Edit ranges overlap or share an ambiguous start offset.');
      }
    }
    const effective = ordered.filter(edit => this.#text.slice(edit.start, edit.end) !== edit.text);
    if (!effective.length) return {accepted: true, changed: false, version: this.version, edits: [], changedRange: null};
    if (this.version === Number.MAX_SAFE_INTEGER) return this.#reject('version-exhausted', 'Source version cannot be incremented safely.');
    ...
    this.#stats.updates++;
    return {
      accepted: true, changed: true, version: this.version, edits: effective,
      changedRange: {start: effective[0].start, oldEnd: effective.at(-1).end, newEnd: effective.at(-1).end + shift},
    };
  }
  ...
  replace(text, {expectedVersion} = {}) {
    const conflict = this.#checkVersion(expectedVersion);
    if (conflict) return conflict;
    if (typeof text !== 'string') return this.#reject('invalid-edit', 'Source text must be a string.');
  ...
  #reject(reason, error) {
    return {accepted: false, changed: false, version: this.version, edits: [], changedRange: null, reason, error};
  }
}

/** Return the smallest single UTF-16 replacement, or null for identical text. */
export function computeSourceEdit(before, after) {
  if (typeof before !== 'string' || typeof after !== 'string') throw new TypeError('Source snapshots must be strings.');
