...
export class ObservableState extends EventTarget {
  ...
  wrap(value, path) {
    ...
    const proxy = new Proxy(value, {
      get: (target, key, receiver) => this.wrap(Reflect.get(target, key, receiver), typeof key === 'string' ? (path ? path + '.' : '') + key : path),
      set: (target, key, next) => {
        if (forbidden.has(key)) throw new Error('Unsafe application data key.');
        next = this.targets.get(next) || next;
        const previous = target[key], existed = Object.hasOwn(target, key);
        const ok = Reflect.set(target, key, next);
        if (ok && (!existed || !Object.is(previous, next))) this.changed(path ? path + '.' + String(key) : String(key));
        return ok;
      },
      deleteProperty: (target, key) => {
        if (forbidden.has(key)) throw new Error('Unsafe application data key.');
        const existed = Object.hasOwn(target, key), ok = Reflect.deleteProperty(target, key);
        if (ok && existed) this.changed(path ? path + '.' + String(key) : String(key));
        return ok;
      },
      defineProperty: (target, key, descriptor) => {
        if (forbidden.has(key) || !Object.hasOwn(descriptor, 'value')) throw new Error('Observable data supports safe value properties.');
        const ok = Reflect.defineProperty(target, key, descriptor);
        if (ok) this.changed(path ? path + '.' + String(key) : String(key));
        return ok;
      },
      setPrototypeOf: () => { throw new Error('Observable data prototypes cannot be changed.'); }
    });
  ...
  get(path = '') { return path ? readPath(this.data, path) : this.data; }
  set(path, value) {
  ...
  batch(action) {
    this.depth++;
    try { return action(this.data); } finally { this.depth--; if (!this.depth) this.flush(); }
  }
  flush() {
  ...
  snapshot() {
    ...
    const copy = value => {
      ...
      seen.set(value, out);
      for (const key of Object.keys(value)) Object.defineProperty(out, key, {value: copy(value[key]), enumerable: true, writable: true, configurable: true});
      return out;
    };
...
export const PROPERTY_PRECEDENCE = Object.freeze(['animation', 'local', 'binding', 'trigger', 'style', 'inherited', 'default']);
...
export function coerceProperty(value, metadata = {}) {
  ...
  if (metadata.coerce) value = metadata.coerce(value);
    ...
    if (typeof value === 'boolean' || String(value).trim() === '' || !Number.isFinite(Number(value))) throw new TypeError('Expected a finite number.');
    value = Number(value);
    if (metadata.minimum !== undefined && value < metadata.minimum || metadata.maximum !== undefined && value > metadata.maximum) throw new RangeError('Property value is outside its allowed range.');
  ...
  if (metadata.values && !metadata.values.includes(value)) throw new RangeError('Property value must be one of: ' + metadata.values.join(', '));
  if (metadata.validate && !metadata.validate(value)) throw new TypeError('Property validation failed.');
  return value;
}
...
export class RuntimePropertyRegistry {
  constructor({builtins = true} = {}) {
    this.types = new Map();
    if (builtins) {
      for (const property of ['Width', 'Height', 'MinWidth', 'MinHeight', 'MaxWidth', 'MaxHeight']) this.register('*', property, {type: 'number', minimum: 0, allowAuto: true});
      for (const property of ['Opacity', 'Value', 'Minimum', 'Maximum', 'FontSize', 'Canvas.Left', 'Canvas.Top', 'Canvas.Right', 'Canvas.Bottom', 'Panel.ZIndex', 'SelectedIndex']) this.register('*', property, {type: 'number', ...(property === 'Opacity' ? {minimum: 0, maximum: 1, defaultValue: 1} : {})});
      for (const property of ['IsEnabled', 'IsVisible', 'IsReadOnly', 'IsChecked']) this.register('*', property, {type: 'boolean', defaultValue: ['IsEnabled', 'IsVisible'].includes(property)});
      for (const property of ['Foreground', 'FontFamily', 'FontSize', 'FontWeight']) this.register('*', property, {...this.get('*', property), inherits: true});
      for (const [type, property] of [['TextBox', 'Text'], ['PasswordBox', 'Text'], ['CheckBox', 'IsChecked'], ['RadioButton', 'IsChecked'], ['ToggleButton', 'IsChecked'], ['ToggleSwitch', 'IsChecked'], ['Slider', 'Value'], ['NumericUpDown', 'Value'], ['ComboBox', 'SelectedIndex'], ['ComboBox', 'SelectedValue'], ['ComboBox', 'SelectedItem'], ['DatePicker', 'SelectedDate']]) this.register(type, property, {...this.get('*', property), defaultBindingMode: 'TwoWay'});
    }
  }
  register(type, property, metadata) {
    if (!type || !property || forbidden.has(property)) throw new Error('A safe control type and property name are required.');
    let properties = this.types.get(type);
    if (!properties) this.types.set(type, properties = new Map());
    const previous = properties.get(property), value = Object.freeze({...metadata});
    properties.set(property, value);
    return () => { if (properties.get(property) === value) { if (previous) properties.set(property, previous); else properties.delete(property); } };
  }
  get(type, property) { return this.types.get(type)?.get(property) || this.types.get(String(type).split(':').at(-1))?.get(property) || this.types.get('*')?.get(property); }
  list(type) { return new Map([...(this.types.get('*') || []), ...(this.types.get(String(type).split(':').at(-1)) || []), ...(this.types.get(type) || [])]); }
}

export class RuntimePropertyStore extends EventTarget {
  constructor(registry = new RuntimePropertyRegistry()) { super(); this.registry = registry; this.values = new Map(); }
  set(nodeId, type, property, value, source = 'local') {
    if (!PROPERTY_PRECEDENCE.includes(source)) throw new Error('Unknown property precedence source.');
  ...
  entries(nodeId) {
    ...
    for (const [property, sources] of this.values.get(nodeId) || []) {
      for (const source of PROPERTY_PRECEDENCE) if (sources.has(source)) { result[property] = sources.get(source); break; }
    }
    return result;
  }
  clear(nodeId, property, source = 'local') {
    const properties = this.values.get(nodeId), sources = properties?.get(property);
  ...
  reset() { this.values.clear(); this.dispatchEvent(eventWith('change', {reset: true})); }
}
