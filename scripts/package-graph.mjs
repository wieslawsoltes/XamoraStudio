...
export async function layout() {
  ...
  if (await exists(resolve(root, 'packages/compiler/package.json'))) {
    ...
    entries.splice(entries.length - 1, 0, {id:'compiler', description:p.description, ...p.xamora});
  }
  return entries;
}
export async function graph({allowMissing=false}={}) {
  const entries = await layout(), owners = new Map(), contents = new Map();
  for (const entry of entries) {
    entry.name = packageName(entry.id); entry.directory = resolve(root, 'packages', entry.id);
    entry.sources = sourceModules(entry); entry.dependencies = new Set(entry.contracts ? [] : ['@wieslawsoltes/xamora-contracts']);
    for (const module of entry.sources) {
      ...
      if (await exists(resolve(root, module.source))) contents.set(module.source, await readFile(resolve(root,module.source),'utf8'));
      else if (!allowMissing) throw Error(`Package ${entry.name} is missing canonical source ${module.source}`);
    }
  }
  const relativeImports = source => [...source.matchAll(/(?:\bfrom\s*|\bimport\s*\(\s*|\bimport\s*)['"]([^'"]+)['"]/g)].map(match=>match[1]);
  for (const [path,source] of contents) for (const specifier of relativeImports(source)) {
    if (!specifier.startsWith('.')) continue;
    const target = slash(relative(root,resolve(root,dirname(path),specifier)));
    const owner = owners.get(target); if (!owner) throw Error(`Unpackaged import ${specifier} from ${path}`);
    const current=owners.get(path).entry; if (owner.entry.id!==current.id) current.dependencies.add(owner.entry.name);
  }
...
export function rewrite(source,path,owners,{cjs=false}={}) {
  ...
  return source.replace(/((?:\bfrom\s*|\bimport\s*\(\s*|\bimport\s*)['"])([^'"]+)(['"])/g,(all,before,value,after)=>{
    ...
    let next;
    if(target.entry.id===current.entry.id){next=slash(relative(dirname(current.name),target.name))+(cjs?'.cjs':'.js');if(!next.startsWith('.'))next='./'+next;}
