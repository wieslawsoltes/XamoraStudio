...
if (
  ...
  throw Error('Use 10–10000 nodes and 1–100 edits.');
const selected = Math.floor(nodes / 2);
const source =
  '<Grid xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation">\n' +
  Array.from(
    { length: nodes },
    (_, i) => `  <Button Name="Button${i}" Content="Value ${i}" Width="120" />`,
  ).join('\n') +
  '\n</Grid>';

function run(incremental) {
  const store = new DocumentStore(parseXaml(source)),
    session = new DocumentSession(store, { source, incremental });
  const target = store.document.root.children.find((n) => n.props?.Name === 'Button' + selected),
    id = target.id;
  const initial = { ...session.processingStats };
  let previous = 'Value ' + selected,
    snapshotEstimate = 0;
  ...
  for (let i = 0; i < edits; i++) {
    ...
    if (!result.accepted || !result.valid || find(store.document.root, id)?.props.Content !== value)
      throw Error('Edit failed or lost target identity.');
    previous = value;
  }
  const elapsed = performance.now() - start,
    counts = Object.fromEntries(
      Object.entries(session.processingStats).map(([key, value]) => [key, value - initial[key]]),
    );
  ...
  session.dispose();
  return {
    mode: incremental ? 'localized parsing' : 'full parsing',
    elapsedMs: Number(elapsed.toFixed(2)),
    averageEditMs: Number((elapsed / edits).toFixed(2)),
    counts,
    history,
    estimatedSnapshotPayloadBytes: snapshotEstimate,
  ...
  JSON.stringify(
    {
      ...
      note: 'CPU timings vary by machine and load. String copies, validation, transient rollback clones and rendering are separate remaining costs. Retention values are estimates, not measured heap usage.',
