...
Popup allocation occurs before layout changes. Blocking, rejected activation, invalid bounds or a failed application portal callback cannot silently remove the requested panels. Returning a host reclaims active and inactive DOM nodes before its document is released. Closing the last group or moving all its panels elsewhere releases only that host. Returning an unrelated host does not cancel another host's pointer gesture or active transfer; a drop preview in the closing destination is removed independently. Foreign or stale-source drag-end events do not cancel the current source transfer. Group movement guards the selected moving member, even when focus is in another group. Disposing the control returns content to its caller and closes its hosts, without deleting the caller-owned model.
...
`DockBrowserWindowOptions.openWindow(features, sourceWindow)` is a testing/host-adapter seam. It must return a **new same-origin blank Window**, or `null` for blocked allocation. Do not return an existing application document. `bodyClass` can supply application ancestor classes required by scoped panel CSS. `onOpen` is for reversible portal integration, not for nested layout batches or panel registration. Disposing the control or manager during allocation, activation or portal setup aborts opening; a cleanup returned after synchronous teardown is still called exactly once. `onClose` may explicitly reopen its floating tree while the owner is live, without the old return clearing the replacement host. Owner `pagehide` suspends new allocations before callbacks run; back/forward-cache restoration makes explicit reopening available again. Disposal releases pending transfers even when no popup is open.
...
`tests/docking-window-routing.test.mjs` verifies document-local menu and Escape routing, modal priority, pointer cancellation ownership and stale portal disposer isolation. `tests/docking-window-isolation.test.mjs` adds unrelated-close/drag-end isolation, selected background-group guards, throwing guards, reentrant allocation/activation/portal teardown, safe onClose reopening, owner navigation, root-input focus and retained-reference cleanup.

`tests/browser-docking-window-state.mjs` qualifies real owner-dialog focus, selection and scroll restoration, invalid detached drafts with shared undo/redo, and full split-tree saved intent/reopening. `tests/browser-docking-isolation.mjs` repeats three-window close/transfer isolation, selected background-group movement and root-input focus against both canonical source and the built standalone browser bundle. These suites await actual popup close events rather than assuming that a removed model record means browser close delivery has completed.
