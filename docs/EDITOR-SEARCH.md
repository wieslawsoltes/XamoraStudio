...
Use **Ctrl/Command+F** or **Edit → Find in source…**. **Ctrl/Command+H** and **Edit → Replace in source…** expose replacement controls. **F3 / Shift+F3** select the next / previous occurrence and wrap at the ends; the status announces wrapping. In the Find field, Enter / Shift+Enter navigate without moving typing focus. Escape closes search and restores focus to the source selection. Buttons retain their own native Enter/Space behavior. Coarse-pointer layouts give search buttons at least 44 by 44 pixels and put the Find field on its own line so the arrow/close targets do not squeeze it. Browser/OS-reserved shortcuts may require using the menu instead.
