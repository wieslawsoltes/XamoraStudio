...
Xamora's packages are built from the same `dist/core` and `dist/controls` modules imported by the designer. The browser studio retains its existing buildless imports; packaging does not replace its parser, object model, renderer, controls, history, or editing tools. The private root workspace is not published. Twelve base packages form the runtime and designer SDK; an optional thirteenth package adds the semantic compiler and CLI.
