...
function compileStub(entries, options) {
  return {
    entries: entries.map((entry) => ({
      ...
      targetPath: entry.path.replace(/\.(xaml|xml|axaml|html?)$/i, '.' + options.to),
      status: 'ready',
      result: {
        source: '<converted>' + (entry.source || entry.document.name) + '</converted>',
        diagnostics: [],
        sourceMap: [],
        losses: [],
      },
...
test('CLI validates complete option values, formats, and write-free dry runs', () => {
  ...
  for (const arguments_ of [
    ...
    ['page.xaml', '--to', 'svg'],
    ['page.xaml', '--to', 'html'],
    ['page.xaml', '--to', 'html', '--dry-run', '--report', 'report.json'],
    ['page.xaml', '--to', 'html', '--dry-run', '--unknown'],
  ]) {
    assert.throws(() => parseArguments(arguments_));
  }
  assert.equal(parseArguments(['--help']).help, true);
  assert.equal(
    parseArguments(['--to', 'html', '--dry-run', '--', '-page.xaml']).input,
    '-page.xaml',
  );
});
...
test('folder scan sorts nested paths, excludes output and dependencies, filters source formats', async (t) => {
  ...
  for (const path of [
    ...
    'views/a/A.axaml',
    'views/Page.html',
    'views/generated/Old.xaml',
    'views/node_modules/Package.xaml',
    'views/.git/Old.xaml',
  ])
    await writeFile(join(directory, path), '<Grid/>');
  await writeFile(join(directory, 'views/readme.md'), 'Ignore');
  const collected = await collectConversionEntries('views', {
    cwd: directory,
    from: 'xaml',
    outDir: 'views/generated',
  });
  assert.deepEqual(
    collected.entries.map((entry) => entry.path),
    ['a/A.axaml', 'z/B.xaml'],
  );
...
test('dry run invokes shared planner but creates neither files nor folders', async (t) => {
  ...
  const result = await invoke(
    directory,
    ['View.xaml', '--to', 'html', '--out-dir', 'output', '--dry-run', '--strict', '--report', '-'],
    {
      planner: (entries, options) => {
        passed = options;
        return compileStub(entries, options);
      },
    },
  );
...
test('writes converted documents with relative directories and a machine-readable report', async (t) => {
  ...
  assert.equal(result.code, 0, result.errors);
  assert.equal(
    await readFile(join(directory, 'out/Views/Page.html'), 'utf8'),
    '<converted><Grid/></converted>',
  );
  assert.equal(
    JSON.parse(await readFile(join(directory, 'out/conversion-report.json'), 'utf8')).success,
    true,
  );
...
test('conversion failure retains diagnostics and writes no partial documents', async (t) => {
  ...
  const result = await invoke(
    directory,
    ['View.xaml', '--to', 'html', '--out-dir', 'out', '--strict', '--report', '-'],
    {
      planner: (entries) => ({
        entries: [
          {
            ...
            status: 'failed',
            diagnostics: [
              { severity: 'warning', code: 'UNMAPPED_CONTROL', message: 'Unknown control' },
            ],
            result: { losses: [{ code: 'UNMAPPED_CONTROL' }] },
          },
        ],
...
test('portable manifests support inline and file sources without escaping their root', async (t) => {
  ...
  await writeFile(
    join(directory, 'solution.json'),
    JSON.stringify({
      format: 'xamora-conversion',
      version: 1,
      files: ['Disk.xaml', { path: 'Nested/Inline.xaml', source: '<Grid/>' }],
    }),
  );
  const result = await collectConversionEntries('solution.json', { cwd: directory });
  assert.deepEqual(
    result.entries.map((entry) => entry.path),
    ['Disk.xaml', 'Nested/Inline.xaml'],
  );
...
test('Xamora workspace manifests pass shared AST documents directly to the planner', async (t) => {
  ...
  const document = {
    ...
    framework: 'Avalonia',
    root: { type: 'Grid' },
    metadata: { solutionPath: 'Views/Page.xaml' },
  };
  await writeFile(
    join(directory, 'workspace.xamora.json'),
    JSON.stringify({ format: 'xamora-workspace', documents: [document] }),
  );
...
test('duplicate case-insensitive manifest paths are rejected before compiling', async (t) => {
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      version: 1,
      files: [
        { path: 'View.xaml', source: '' },
        { path: 'view.xaml', source: '' },
      ],
    }),
  );
  await assert.rejects(
    collectConversionEntries('solution.json', { cwd: directory }),
...
test('symlinks are skipped during recursion and rejected as manifest sources or output parents', async (t) => {
  ...
  await writeFile(
    join(directory, 'solution.json'),
    JSON.stringify({ format: 'xamora-conversion', version: 1, files: ['src/link/Other.xaml'] }),
  );
  await assert.rejects(
    collectConversionEntries('solution.json', { cwd: directory }),
    /symbolic links/,
...
test('compiler target traversal and duplicate targets are rejected without writing', async (t) => {
  const directory = await workspace(t);
  await writeFile(join(directory, 'View.xaml'), '<Grid/>');
  const result = await invoke(directory, ['View.xaml', '--to', 'html', '--out-dir', 'out'], {
    planner: () => ({
      entries: [
        { sourcePath: 'View.xaml', targetPath: '../escape.html', status: 'ready', source: 'No' },
      ],
      summary: { ready: 1 },
    }),
  });
  assert.equal(result.code, 2);
  assert.deepEqual(await readdir(directory), ['View.xaml']);
  const duplicates = await invoke(directory, ['View.xaml', '--to', 'html', '--out-dir', 'out'], {
    planner: () => ({
      entries: ['a.html', 'A.html'].map((targetPath) => ({
        sourcePath: 'View.xaml',
        targetPath,
        status: 'ready',
        source: 'No',
      })),
...
test('a report path cannot overwrite an input or collide with generated output', async (t) => {
  ...
  const collision = await invoke(directory, [
    'View.xaml',
    '--to',
    'html',
    '--out-dir',
...
test('unreadable manifest file aborts collection without output', async (t) => {
  ...
  await writeFile(
    ...
    JSON.stringify({
      format: 'xamora-conversion',
      version: 1,
      files: [{ path: 'Good.xaml', source: '<Grid/>' }, 'Missing.xaml'],
    }),
  );
...
test('invalid UTF-8 input fails before writing and preserves source bytes', async (t) => {
  ...
  const invalid = Buffer.from([
    0x3c, 0x47, 0x72, 0x69, 0x64, 0x3e, 0xc3, 0x28, 0x3c, 0x2f, 0x47, 0x72, 0x69, 0x64, 0x3e,
  ]);
...
test('absolute output roots preserve unrelated files and dry-run also checks the default report', async (t) => {
  ...
  const dryRun = await invoke(directory, [
    'View.xaml',
    '--to',
    'html',
    '--out-dir',
  ...
  assert.deepEqual((await readdir(join(directory, 'out'))).sort(), [
...
test('real compiler CLI parses source and emitted HTML in both directions', async (t) => {
  const directory = await workspace(t);
  await writeFile(
    join(directory, 'View.xaml'),
    '<Grid xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"><Button Content="Original" Width="120"/></Grid>',
  );
  const toHtml = await invoke(directory, ['View.xaml', '--to', 'html', '--out-dir', 'web'], {
    planner: undefined,
  });
  ...
  const toXaml = await invoke(directory, ['web/View.html', '--to', 'xaml', '--out-dir', 'native'], {
    planner: undefined,
  });
  ...
  assert.equal(
    document.root.children.find((node) => node.type === 'Button').props.Content,
    'Edited',
  );
});
...
test('real compiler copies nested referenced binary assets into the output root', async (t) => {
  ...
  await writeFile(
    join(directory, 'src/Views/Page.xaml'),
    '<Grid xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"><Image Source="../images/photo.png"/></Grid>',
  );
  const result = await invoke(directory, ['src', '--to', 'html', '--out-dir', 'out'], {
    planner: undefined,
  });
  ...
  assert.equal(
    JSON.parse(await readFile(join(directory, 'out/conversion-report.json'), 'utf8')).assets[0]
      .bytes,
    image.length,
  );
});
...
test('asset graph follows nested CSS references while refusing missing files and escaping paths', async (t) => {
  ...
  await writeFile(
    join(directory, 'styles/site.css'),
    '@import "theme/colors.css"; main { background: url(../images/back.png) }',
  );
  await writeFile(
    join(directory, 'styles/theme/colors.css'),
  ...
  const result = await collectReferencedAssets(
    {
      entries: [
        {
          status: 'ready',
  ...
  assert.deepEqual(
    ...
    ['images/back.png', 'images/icon.png', 'styles/site.css', 'styles/theme/colors.css'],
  );
...
test('strict asset failures abort the batch and preserve source bytes', async (t) => {
  const directory = await workspace(t);
  const source =
    '<Grid xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"><Image Source="missing.png"/></Grid>';
...
test('HTML parsing does not evaluate scripts during conversion', async (t) => {
  ...
  await writeFile(
    ...
    '<!DOCTYPE html><html><head><script>throw new Error("MUST NOT EXECUTE");</script></head><body><button>Safe</button></body></html>',
  );
  const result = await invoke(directory, ['page.html', '--to', 'xaml', '--out-dir', 'out'], {
    planner: undefined,
  });
  assert.equal(result.code, 0, result.errors);
...
test('executable entrypoint works through npm-style bin symlinks from another directory', async (t) => {
  const directory = await workspace(t);
  await symlink(
    fileURLToPath(new URL('../dist/compiler-cli/index.js', import.meta.url)),
    join(directory, 'xamora-convert'),
  );
  const result = await promisify(execFile)(
    process.execPath,
    [join(directory, 'xamora-convert'), '--help'],
    { cwd: directory },
  );
