...
try {
  ...
  const place = async (needle, inside = needle) => {
    await input.focus();
    await input.evaluate(
      (el, { needle, inside }) => {
        const at = el.value.indexOf(needle) + needle.indexOf(inside) + 1;
        if (at < 1) throw Error('Missing token ' + needle);
 ...
 <EventTrigger SourceName='Save'/>
  ...
  assert.equal(
    ...
    referenceCaret,
    'navigate back returns exact source position',
  );
  await input.press('Shift+F12');
  await page.waitForFunction(
    () => document.querySelectorAll('.semantic-results [data-semantic-result]').length === 3,
  );
  assert.equal(
    await page.locator('.semantic-summary').textContent(),
    '3 references · Language.xaml',
  );
  await place('ElementName=Save', 'Save');
  const history = await page.evaluate(() => window.xamora.studio.store.history.length);
  await input.press('F2');
  await page.getByLabel('New symbol name').fill('Commit');
  await page.locator('[data-modal-action="0"]').click();
  await page.waitForFunction(() =>
    window.xamora.studio.store.session.source.includes("x:Name='Commit'"),
  );
  const renamed = await input.inputValue();
  assert.ok(renamed.includes('ElementName=Commit'));
  assert.ok(renamed.includes("SourceName='Commit'"));
  assert.ok(renamed.includes("Content='Save'"));
  assert.ok(renamed.includes('Save is literal commentary'));
  assert.equal(
    await page.evaluate(() => window.xamora.studio.store.history.length),
    history + 1,
    'rename adds one shared history transaction',
  );
  await input.focus();
  await input.press('Control+z');
  assert.equal(await input.inputValue(), xml, 'source undo reverses all rename edits together');
  assert.ok(
    (await page.evaluate(() => window.xamora.language.diagnostics())).every(
      (issue) => issue.code !== 'unresolved-local-reference',
    ),
  );
  console.log(
    'PASS semantic XAML: F12, references panel, rename dialog, capture-safe local references, exact undo, back navigation',
  );
  ...
  await page.evaluate((source) => {
    const s = window.xamora.studio;
    s.importText(source, 'Language.html');
    s.setView('split');
    s.store.session.updateSource(source);
  }, html);
  await page.waitForFunction(
    () =>
      ...
      !!window.xamora.studio.renderer.htmlRenderer?.frame?.contentDocument.querySelector('#action'),
  );
  await place("href='#action'", 'action');
  await input.press('F12');
  assert.equal(
    await input.evaluate((el) => el.value.slice(el.selectionStart, el.selectionEnd)),
    'action',
  ...
  await input.press('F2');
  await page.getByLabel('New symbol name').fill('submitAction');
  await page.locator('[data-modal-action="0"]').click();
  await page.waitForFunction(
    () =>
      !window.xamora.studio.sync.renderFrame &&
  ...
  const renamedHtml = await input.inputValue();
  assert.ok(renamedHtml.includes("for='submitAction'"));
  assert.ok(renamedHtml.includes("href='#submitAction'"));
  assert.ok(renamedHtml.includes('#submitAction {'));
  assert.ok(renamedHtml.includes('/* #action stays in a comment */'));
  assert.ok(renamedHtml.includes("const text='#action'"));
  await input.focus();
  await input.press('Control+z');
  assert.equal(await input.inputValue(), html);
  console.log(
    'PASS semantic HTML: fragment navigation, ID and selector rename, literal-script preservation, one-step undo',
  );

  await page.evaluate(() => {
    const s = window.xamora.studio;
    s.registry.registerControl({
      type: 'Meter',
      category: 'Custom',
      frameworks: ['WPF'],
  ...
  await input.evaluate((el) => {
    const at = el.value.indexOf('Li') + 2;
    el.setSelectionRange(at, at);
  });
  ...
  await page.waitForFunction(
    () =>
      ...
      document.querySelector('.completions').textContent.includes('Linear'),
  );
  await input.press('Enter');
  await page.waitForFunction(() => window.xamora.studio.doc.root.props.ScaleMode === 'Linear');
  assert.ok(await input.evaluate((el) => el.value.includes('ScaleMode="Linear"')));
  await page.evaluate(() => {
    const s = window.xamora.studio;
    s.editor.input.value = '<Meter';
    s.editor.changed();
  });
  await input.focus();
  await input.press('F2');
  assert.equal(
    await page.locator('#semantic-rename-name').count(),
    0,
    'invalid draft cannot trigger rename',
  );
  await page.evaluate(() => window.xamora.studio.store.session.discardDraft());
  await page.waitForFunction(() => !window.xamora.studio.sync.renderFrame);
  const diagnosticSource =
    '<Grid>\n <Button Content="First"/>\n <UnregisteredDiagnosticWidget/>\n</Grid>';
  const oldDiagnostic = await page.evaluate((source) => {
    const s = window.xamora.studio;
    s.importText(source, 'Diagnostic.xaml');
    s.setView('split');
    s.store.session.updateSource(source);
    const node = s.doc.root.children.find((node) => node.type === 'UnregisteredDiagnosticWidget');
    return { id: node.id, line: s.store.session.sourceAtNode(node.id).line };
  }, diagnosticSource);
  await input.focus();
  await input.evaluate((el) => {
    const index = el.value.indexOf('First');
    el.setSelectionRange(index, index + 5);
  });
  await page.keyboard.insertText('First\nSecond');
  await page.waitForFunction((id) => {
    const s = window.xamora.studio,
      issue = s.issues.find((issue) => issue.id === id),
      span = s.store.session.sourceAtNode(id);
    return !s.sync.renderFrame && issue && issue.line === span.line;
  }, oldDiagnostic.id);
  const movedDiagnostic = await page.evaluate((id) => {
    const s = window.xamora.studio;
    return {
      actual: s.issues.find((issue) => issue.id === id).line,
      expected: s.store.session.sourceAtNode(id).line,
    };
  }, oldDiagnostic.id);
  assert.equal(
    movedDiagnostic.actual,
    oldDiagnostic.line + 1,
    'legacy diagnostic line moves after multiline attribute editing',
  );
  assert.equal(movedDiagnostic.actual, movedDiagnostic.expected);
  console.log(
  ...
  if (page) {
    ...
    await writeFile(
      'test-results/browser-language-failure.json',
      JSON.stringify(diagnostics, null, 2),
    );
    await page
      .screenshot({ path: 'test-results/browser-language-failure.png', fullPage: true })
      .catch(() => {});
