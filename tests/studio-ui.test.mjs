...
import { $, $$, esc, createNotifier, notify, toast, saveFile, download } from '../dist/studio/ui.js';
...
test('shared escaping and selectors preserve nullish values, entities and explicit roots', t => {
  ...
  const root = { querySelector: selector => { assert.equal(selector, '.item'); return element; }, querySelectorAll: () => nodes };
...
test('download aliases preserve MIME defaults, filenames, content and delayed URL cleanup', async t => {
  ...
  fakeDocument(t, { createElement: tag => { assert.equal(tag, 'a'); const link = { click() { this.clicked = true; } }; links.push(link); return link; } });
  saveFile('workspace.json', '{"hello":true}');
  download('view.xaml', '<Grid/>');
  download('view.html', '<p>Hello</p>', 'text/html');
  assert.deepEqual(blobs.map(blob => blob.type), ['application/json', 'text/plain', 'text/html']);
  assert.deepEqual(await Promise.all(blobs.map(blob => blob.text())), ['{"hello":true}', '<Grid/>', '<p>Hello</p>']);
  assert.deepEqual(links.map(link => [link.href, link.download, link.clicked]), [['blob:1', 'workspace.json', true], ['blob:2', 'view.xaml', true], ['blob:3', 'view.html', true]]);
