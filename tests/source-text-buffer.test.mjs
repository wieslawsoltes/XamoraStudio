...
function assertIndex(buffer) {
  ...
  for (let offset = 0; offset <= buffer.length; offset++) {
    ...
    assert.deepEqual(buffer.positionAt(offset), {line: line + 1, column: offset - expected[line] + 1});
...
test('unsorted original-snapshot edits apply atomically and report the combined range', () => {
  ...
  const result = buffer.applyEdits([{start: 9, end: 12, text: 'z'}, {start: 1, end: 2, text: 'XY'}], {expectedVersion: 0});
  assert.equal(buffer.text, 'aXYc\r\ndef\nz');
  assert.equal(result.accepted, true);
  assert.equal(result.version, 1);
  assert.deepEqual(result.changedRange, {start: 1, oldEnd: 12, newEnd: 11});
  assert.deepEqual(result.edits.map(edit => edit.start), [1, 9]);
  assertIndex(buffer);
});

test('stale, malformed and overlapping batches leave text, index, version and counters unchanged', () => {
  const buffer = new SourceTextBuffer('abc\r\ndef');
  const original = {text: buffer.text, starts: buffer.lineStarts, stats: buffer.stats, version: buffer.version};
  const invalid = [null, [{start: -1, end: 0, text: ''}], [{start: 2, end: 1, text: ''}], [{start: 0, end: 99, text: ''}], [{start: 0.5, end: 1, text: ''}], [{start: 0, end: 1, text: null}], [{start: 0, end: 3, text: ''}, {start: 2, end: 4, text: ''}], [{start: 1, end: 1, text: 'a'}, {start: 1, end: 1, text: 'b'}]];
  for (const edits of invalid) assert.equal(buffer.applyEdits(edits).accepted, false);
  assert.equal(buffer.applyEdits([{start: 0, end: 1, text: 'z'}], {expectedVersion: 1}).reason, 'version-conflict');
  assert.equal(buffer.replace('xyz', {expectedVersion: 2}).accepted, false);
  assert.equal(buffer.replace(null).accepted, false);
  assert.deepEqual({text: buffer.text, starts: buffer.lineStarts, stats: buffer.stats, version: buffer.version}, original);
});
...
test('snapshots and exposed indexes are independent, and positions clamp consistently', () => {
  ...
  buffer.lineStarts.push(999); buffer.stats.fullResets = 999;
  assert.equal(buffer.stats.fullResets, 1);
  ...
  assertIndex(buffer); assertIndex(fork);
});
...
test('deterministic random batches match full text and line rescans after every transaction', () => {
  ...
  const fragment = () => Array.from({length: random(7)}, () => alphabet[random(alphabet.length)]).join('');
  let text = '\uFEFF<Grid>😀\r\nabc\rdef\n</Grid>';
  const buffer = new SourceTextBuffer(text);
  for (let iteration = 0; iteration < 600; iteration++) {
    ...
    const edits = candidates.filter((edit, index, all) => index === 0 || edit.start > Math.max(...all.slice(0, index).map(previous => previous.end)));
    for (const edit of edits.toReversed()) text = text.slice(0, edit.start) + edit.text + text.slice(edit.end);
