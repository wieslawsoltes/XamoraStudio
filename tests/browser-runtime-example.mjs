...
const server = createServer(async (request, response) => {
  try {
    ...
    if (pathname === '/package-runtime/') {
      response.writeHead(200, {'Content-Type': 'text/html', 'Cache-Control': 'no-store'}).end('<!doctype html><html><head><title>Packed runtime test</title><link rel="icon" href="data:,"></head><body><main id="packed-app"></main><script type="module">window.bundleRuntime = await import("/package-runtime/index.js");</script></body></html>');
      return;
    }
    if (pathname === '/package-runtime/index.js') {
      response.writeHead(200, {'Content-Type': 'text/javascript', 'Cache-Control': 'no-store'}).end(await readFile(browserBundle));
      return;
    }
    let file = resolve(root, '.' + pathname);
    if (file !== root && !file.startsWith(root + sep)) { response.writeHead(403).end(); return; }
    if ((await stat(file)).isDirectory()) file = resolve(file, 'index.html');
    response.writeHead(200, {'Content-Type': mime[extname(file)] || 'text/plain', 'Cache-Control': 'no-store'}).end(await readFile(file));
...
try {
  ...
  await page.waitForFunction(() => window.launchboard.application.data.Project.Title === 'Browser launch');
  assert.equal(await page.getByText('Browser launch', {exact: true}).count(), 1, 'text binding refreshed from the input');
  ...
  await page.waitForFunction(() => window.launchboard.application.data.Project.Owner === 'Taylor Reed');
  ...
  console.log('PASS standalone example: input-to-model-to-view, custom control, item-template edits, commands and canExecute');

  const colorBefore = await page.getByRole('button', {name: 'Preview celebration', exact: true}).evaluate(node => getComputedStyle(node).backgroundColor);
  ...
  const colorAfter = await page.getByRole('button', {name: 'Preview celebration', exact: true}).evaluate(node => getComputedStyle(node).backgroundColor);
  ...
  await page.waitForFunction(() => Number(getComputedStyle(window.launchboard.application.findName('SummaryCard')).opacity) < .5);
  await page.getByRole('button', {name: 'Toggle state', exact: true}).click();
  await page.waitForFunction(() => Number(getComputedStyle(window.launchboard.application.findName('SummaryCard')).opacity) > .99);
  ...
  const motion = await page.evaluate(() => {
    ...
    player.pause(); player.seek(.15);
    return {count: app.players.size, opacity: Number(getComputedStyle(app.findName('SummaryCard')).opacity)};
  });
  assert.ok(motion.count > 0);
  assert.ok(motion.opacity > .45 && motion.opacity < 1, 'storyboard samples a visible intermediate opacity');
  console.log('PASS standalone example: live resources/theme, generated state transitions and storyboard sampling');
  ...
  assert.equal(await page.getByTitle('Project name', {exact: true}).inputValue(), 'Browser launch');
  assert.equal(await page.getByTitle('Project owner', {exact: true}).inputValue(), 'Taylor Reed');
  await page.locator('#data-tab').focus(); await page.keyboard.press('ArrowRight');
  ...
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth + 1), true, 'example has no page overflow on a narrow viewport');
  await page.setViewportSize({width: 1440, height: 1100});
  assert.ok(!requests.some(path => path === '/app.js' || path.startsWith('/studio/') || /^\/styles\/(studio|features|editor|docking|motion|density)\.css$/.test(path)), 'standalone example loads no studio entry point, modules or styles');
  console.log('PASS standalone example: disposal/remount, retained data, keyboard tabs, compact viewport and no studio dependencies');
  ...
  const lifecycle = await page.evaluate(async () => {
    ...
    const source = '<StackPanel xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml"><TextBox x:Name="Editor" Text="{Binding Name, Mode=TwoWay, UpdateSourceTrigger=PropertyChanged}"/><TextBlock x:Name="Label" Text="{Binding Name}"/></StackPanel>';
    registerXamlElement('test-xaml-view', {source, data: {Name: 'Default'}});
    const first = document.createElement('test-xaml-view'), second = document.createElement('test-xaml-view');
    ...
    for (let index = 0; index < 4; index++) {
      ...
      first.remove();
      check(first.application === null && previous.disposed, 'disconnect disposes previous application');
      document.body.append(first);
      await settle();
      check(first.application.state === state, 'reconnect reuses ObservableState without wrapping old proxies');
      first.data.Name = 'Reconnect ' + index;
      await settle();
      check(first.application.findName('Label').textContent === 'Reconnect ' + index, 'reconnected binding remains observable');
    }
    const snapshot = state.snapshot();
    await first.reload();
    check(first.application.state === state && first.data.Name === snapshot.Name, 'reload keeps the existing observable state');
    const live = first.application;
    let failure;
    first.addEventListener('xamora-error', event => { failure = event.detail.error; });
    first.source = '<Grid';
    await settle();
    check(failure && first.application === live && !live.disposed, 'invalid replacement reports error and preserves mounted application');
    first.source = source;
    await settle();
    ...
    await settle();
    check(first.application.findName('Label').textContent === 'Together' && second.application.findName('Label').textContent === 'Together', 'explicit ObservableState sharing synchronizes hosts');
    ...
    const script = document.createElement('script'); script.type = 'application/xaml'; script.textContent = '<TextBlock Text="{Binding Name}"/>';
    inline.append(script); document.body.append(inline);
    await settle();
    check(inline.shadowRoot.textContent.includes('Before connect'), 'pre-connect data mutations survive mounting inline source');
    script.textContent = '<TextBlock Text="Reloaded inline source"/>';
    await inline.reload();
    check(inline.shadowRoot.textContent.includes('Reloaded inline source'), 'reload rereads inline source');
    inline.remove();
    const pending = new Map();
    const fetcher = (url, {signal}) => new Promise(resolve => pending.set(new URL(url).pathname, {resolve, signal}));
    ...
    check(fetched.shadowRoot.textContent.includes('Second') && !fetched.shadowRoot.textContent.includes('Stale'), 'late response cannot replace current source even if fetch ignores abort');
    fetched.setAttribute('src', '/third.xaml');
    fetched.remove();
    pending.get('/third.xaml').resolve(new Response('<TextBlock Text="Detached"/>'));
    await settle();
    check(fetched.application === null && pending.get('/third.xaml').signal.aborted, 'disconnection aborts in-flight fetch and prevents remount');
  ...
  assert.equal(await shadowInput.inputValue(), 'Shadow typing', 'successive typing retains focus through shadow-root refreshes');
  await page.evaluate(() => document.querySelector('typing-xaml-view').remove());
  assert.deepEqual(await page.evaluate(() => window.launchboard.application.diagnostics.filter(issue => issue.severity === 'error')), []);
  assert.deepEqual(errors, [], 'no unhandled browser errors');
  console.log('PASS custom element: shadow host, independent defaults, repeated reconnect, retained state, invalid-source recovery, explicit state sharing, stale fetch prevention and focus retention');
  ...
  await bundled.evaluate(() => {
    ...
    window.bundledApp = createApplication({
      ...
      commands: {Increment: (_parameter, {data}) => {data.Count++;}},
      plugins: [{setup(app) {
        return app.registry.registerControl({type: 'pkg:Badge', category: 'Package test', render({properties}) {
          const badge = document.createElement('span'); badge.className = 'package-badge'; badge.textContent = properties.Text; return badge;
        }});
      }}]
    });
  ...
  await bundled.waitForFunction(() => window.bundledApp.data.Value === 'One browser bundle' && window.bundledApp.findName('CountLabel').textContent === '1');
  ...
  await bundled.waitForFunction(() => getComputedStyle(document.querySelector('#packed-app button')).backgroundColor === 'rgb(101, 66, 164)');
  await bundled.evaluate(async () => {
    const {registerXamlElement} = window.bundleRuntime;
    registerXamlElement('packed-xaml-view', {source: '<TextBlock Text="{Binding Value}"/>', data: {Value: 'Bundled custom element'}});
    const host = document.createElement('packed-xaml-view'); document.body.append(host);
    await new Promise(resolve => requestAnimationFrame(resolve));
    if (!host.shadowRoot.textContent.includes('Bundled custom element')) throw new Error('Bundled custom element did not mount.');
    const app = host.application; host.remove();
    if (!app.disposed) throw new Error('Bundled custom element did not dispose.');
    window.bundledApp.dispose();
  });
  assert.deepEqual([...new Set(bundledRequests)].sort(), ['/package-runtime/', '/package-runtime/index.js'], 'browser artifact runs without package resolution or additional runtime/studio modules');
  await bundled.close();
  assert.deepEqual(errors, [], 'standalone source and bundled entry have no unhandled browser errors');
  console.log('PASS browser package entry: one bundle, two-way binding, command, resource update, custom control and custom-element lifecycle without external modules');
  console.log('Standalone runtime browser integration passed.');
} catch (error) {
  if (page) {
    await mkdir('test-results', {recursive: true});
    await writeFile('test-results/browser-runtime-example-failure.json', JSON.stringify({error: error.stack || error.message, errors, body: await page.locator('body').innerText()}, null, 2));
    await page.screenshot({path: 'test-results/browser-runtime-example-failure.png', fullPage: true}).catch(() => {});
  }
  throw error;
} finally {
  await browser?.close();
