...
try{
  ...
  await page.waitForFunction(id=>{const s=window.xamora.studio;return s.selected.find(n=>n.id===id)?.props.Content==='Typed'&&s.store.session.isValid;},xaml.nodeId);
  ...
  await page.waitForFunction(()=>window.xamora.studio.selected[0]?.props.Width==='180');
  ...
  await page.waitForFunction(()=>window.xamora.studio.selected[0]?.props.Content==='Before');
  await input.press('Control+Shift+z');
  await page.waitForFunction(()=>window.xamora.studio.selected[0]?.props.Content==='Typed');
  ...
  await page.waitForFunction(id=>{const s=window.xamora.studio;return s.renderer.htmlRenderer?.elements.get(id)?.textContent==='Typed HTML';},htmlId);
  ...
  await page.waitForFunction(id=>Math.round(window.xamora.studio.renderer.htmlRenderer?.elements.get(id)?.getBoundingClientRect().width)===190,htmlId);
  await page.evaluate(()=>window.xamora.studio.command('undo'));
  assert.equal(await input.inputValue(),htmlTyped,'HTML property undo returns typed text with original quotes');
  await input.focus();await input.press('Control+z');
  await page.waitForFunction(id=>window.xamora.studio.renderer.htmlRenderer?.elements.get(id)?.textContent==='Before',htmlId);
  await input.press('Control+Shift+z');
  await page.waitForFunction(id=>window.xamora.studio.renderer.htmlRenderer?.elements.get(id)?.textContent==='Typed HTML',htmlId);
  ...
  await input.fill(htmlTyped);await page.waitForFunction(()=>window.xamora.studio.store.session.isValid);
  console.log('PASS HTML: native parser, typed source, iframe render, CSS inspector, shared history, malformed-source retention');
