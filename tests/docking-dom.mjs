...
export class DockElement extends FakeElement {
  ...
  get innerHTML() {
    return this._html || super.innerHTML;
  }
  getAttribute(key) {
    if (key === 'id') return this.id || null;
    if (key === 'class') return this.className;
    if (key === 'hidden') return this.hidden ? '' : null;
    if (key === 'tabindex') return this.tabIndex === undefined ? null : String(this.tabIndex);
    if (key.startsWith('data-'))
      return this.dataset[key.slice(5).replace(/-([a-z])/g, (_, c) => c.toUpperCase())] ?? null;
  ...
  matches(selectors) {
    return selectors.split(/,(?![^[]*\])/).some((selector) => {
      ...
      const tokens = parts(selector);
      if (tokens.length > 1) {
        const last = tokens.pop();
        if (!this.matches(last)) return false;
        let n = this.parentElement;
        const parent = tokens.join(' ');
        while (n) {
          if (n.matches(parent)) return true;
          n = n.parentElement;
        }
        return false;
      }
      const not = [...selector.matchAll(/:not\(([^)]+)\)/g)];
      for (const m of not) if (this.matches(m[1])) return false;
  ...
  removeEventListener(name, listener) {
    this.listeners[name] = (this.listeners[name] || []).filter((fn) => fn !== listener);
  }
...
export function installDockDOM() {
  ...
  let frame = 0;
  doc = {
    createElement: (tag) => new DockElement(tag),
    createElementNS: (ns, tag) => new DockElement(tag),
    createTextNode: (text) => {
      const n = new DockElement('text');
      n.textContent = text;
      return n;
    },
    activeElement: null,
    ...
    querySelector(s) {
      return this.body.matches(s) ? this.body : this.body.querySelector(s);
    },
    querySelectorAll(s) {
    ...
    elementFromPoint() {
      return this.hit || this.body;
    },
  };
  doc.body = new DockElement('body');
  doc.activeElement = doc.body;
  const toast = new DockElement();
  toast.id = 'toast';
  doc.body.append(toast);
  globalThis.document = doc;
  globalThis.Element = globalThis.HTMLElement = DockElement;
  globalThis.window = {
    innerWidth: 1440,
    innerHeight: 900,
    xamora: {},
  ...
  return {
    ...
    frames,
    storage,
    element(tag = 'div', id, cls) {
      const n = new DockElement(tag);
      if (id) n.id = id;
