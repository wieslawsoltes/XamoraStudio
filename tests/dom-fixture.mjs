...
class FakeClassList {
  ...
  toggle(n, force) {
    ...
    on ? this.set.add(n) : this.set.delete(n);
    return on;
  }
  toString() {
...
export class FakeElement {
  ...
  set className(v) {
    this.classList.set = new Set(v.split(/\s+/));
  }
  get className() {
  ...
  removeAttribute(k) {
    ...
    if (k.startsWith('data-'))
      delete this.dataset[k.slice(5).replace(/-([a-z])/g, (_, c) => c.toUpperCase())];
  }
  ...
  matches(selector) {
    return selector.split(',').some((s) => {
      ...
      return s.startsWith('.')
        ...
        : s === '[data-node-id]'
          ? this.dataset.nodeId !== undefined
          : s.startsWith('[')
            ? Object.hasOwn(this.attributes, s.slice(1, -1))
  ...
  querySelectorAll(selectors) {
    const out = [],
      list = selectors.split(',').map((s) => s.trim());
    const matches = (n, s) =>
      s.startsWith('.')
        ? n.classList?.contains(s.slice(1))
        : s.startsWith('[')
          ? s === '[data-node-id]'
            ? n.dataset?.nodeId !== undefined
            : Object.hasOwn(n.attributes || {}, s.slice(1, -1))
          : n.tagName?.toLowerCase() === s;
    const visit = (n) => {
      for (const c of n.children || []) {
        if (list.some((s) => matches(c, s))) out.push(c);
        visit(c);
      }
    };
    visit(this);
  ...
  get innerHTML() {
    return (
      encode(this._text) +
      this.children
        .map((n) => (n instanceof FakeElement ? n.outerHTML : encode(n.textContent)))
        .join('')
    );
  }
  get outerHTML() {
    ...
    for (const [key, value] of Object.entries(this.dataset))
      props['data-' + key.replace(/[A-Z]/g, (c) => '-' + c.toLowerCase())] = value;
