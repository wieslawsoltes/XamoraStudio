...
function fixture(source = original) {
  ...
  return {
    store,
    session,
    primary: store.document.root.children.find((n) => n.props?.['x:Name'] === 'Primary'),
  };
}

test('code, inspector, canvas insertion and document history share one selected model', () => {
  const { store, session, primary } = fixture();
  ...
  store.undo();
  assert.equal(find(store.document.root, label.id), null);
  assert.ok(!session.source.includes('From canvas'));
  store.undo();
  assert.equal(find(store.document.root, primary.id).props.Width, '120');
  assert.equal(session.source, typed);
  store.undo();
  assert.equal(session.source, original);
  assert.equal(find(store.document.root, primary.id).props.Content, 'Before');
  store.redo();
  store.redo();
  store.redo();
  assert.equal(find(store.document.root, label.id).props.Text, 'From canvas');
  session.dispose();
});

test('invalid source survives unrelated document changes and visual mutation rejects without losing the draft', () => {
  const a = fixture(),
    b = fixture('<Canvas Width="900"/>');
  const invalid = original.replace('</Grid>', '');
  const before = clone(a.store.document.root);
  const result = a.session.updateSource(invalid);
  assert.equal(result.accepted, true);
  assert.equal(result.valid, false);
  assert.equal(a.session.source, invalid);
  ...
  assert.equal(a.session.source, invalid);
  assert.ok(b.session.source.includes('901'));
...
test('serialized solution document restores both valid syntax and an invalid draft', () => {
  const { store, session, primary } = fixture();
  store.setProperty([primary.id], 'Height', '42');
  const valid = session.source;
  session.updateSource(valid.replace('</Grid>', ''));
  ...
  assert.equal(restored.source, session.source);
  assert.equal(restored.validSource, valid);
  ...
  assert.equal(restored.source, valid);
...
test('source map follows structural moves and deletion without retaining stale locations', () => {
  ...
  const container = element('StackPanel', { 'x:Name': 'Destination' });
  store.insert(root.id, container);
  store.move([primary.id], container.id);
  const span = session.sourceAtNode(primary.id);
  assert.ok(span);
  assert.ok(session.source.slice(span.start, span.end).includes("x:Name='Primary'"));
  assert.equal(session.nodeAtOffset(span.start + 2)?.id, primary.id);
  store.remove([primary.id]);
  assert.equal(session.sourceAtNode(primary.id), null);
  store.undo();
  assert.equal(session.nodeAtOffset(session.sourceAtNode(primary.id).start + 2)?.id, primary.id);
  session.dispose();
});
...
test('revision-checked extension edit rejects stale source without overwriting panel changes', () => {
  ...
  store.setProperty([primary.id], 'Width', '200');
  const latest = session.source;
  const result = session.updateSource(original.replace("'Before'", "'Stale'"), {
    expectedRevision: revision,
  });
  assert.equal(result.accepted, false);
  assert.equal(session.source, latest);
  assert.equal(find(store.document.root, primary.id).props.Width, '200');
  const fresh = session.updateSource(latest.replace("'Before'", "'Fresh'"), {
    expectedRevision: session.revision,
  });
  assert.equal(fresh.accepted, true);
  assert.equal(find(store.document.root, primary.id).props.Content, 'Fresh');
  session.dispose();
});

test('caret mapping keeps a selected unchanged token through a remote property patch', () => {
  const before = '<Button Width="20" Content="Caret stays here" />',
    after = before.replace('"20"', '"2000"');
  const start = before.indexOf('Caret'),
    end = start + 5;
  const mapped = mapTextSelection(before, after, start, end);
  assert.equal(after.slice(mapped.start, mapped.end), 'Caret');
  assert.equal(mapped.start, start + 2);
  assert.equal(mapped.end, end + 2);
});
...
test('store listeners observe matching source and tree after gesture snapshot commits', () => {
  ...
  store.addEventListener('change', () => {
    const parsed = parseXaml(session.validSource);
    observations.push({
      model: find(store.document.root, primary.id).props.Width,
      source: parsed.root.children.find((n) => n.props?.['x:Name'] === 'Primary').props.Width,
      revision: session.revision,
    });
  });
  const before = clone(store.document);
  find(store.document.root, primary.id).props.Width = '220';
  store.commitSnapshot('Resize gesture', before);
  assert.equal(observations.length, 1);
  assert.equal(observations[0].model, '220');
  assert.equal(observations[0].source, '220');
  assert.equal(observations[0].revision, store.revision);
  store.undo();
  assert.equal(observations[1].model, '120');
  assert.equal(observations[1].source, '120');
  session.dispose();
});
...
test('source editing a named element across containers preserves selected ID and unrelated metadata', () => {
  ...
  const store = new DocumentStore(parseXaml(source)),
    ...
    id = store.document.root.children[0].children[0].id;
  store.document.annotations = [{ id: 'annotation', targetId: id, text: 'Keep this relationship' }];
  store.select([id]);
  ...
  assert.equal(session.updateSource(moved).valid, true);
  assert.deepEqual(store.selection, [id]);
  assert.equal(find(store.document.root, id).props.Content, 'Moved');
  assert.equal(store.document.root.children[1].children[0].id, id);
  assert.equal(store.document.annotations[0].targetId, id);
  session.dispose();
});

test('root code dimensions update the artboard and shared undo restores its prior viewport', () => {
  const { store, session } = fixture(
    '<Canvas Width="640" Height="420"><Button Content="Hi"/></Canvas>',
  );
  store.document.design.zoomHint = 1.25;
  store.document.design.height = 500;
  session.updateSource('<Canvas Width="800" Height="420"><Button Content="Hi"/></Canvas>');
  assert.equal(store.document.root.props.Width, '800');
  assert.equal(store.document.design.width, 800);
  assert.equal(
    store.document.design.height,
    500,
    'unchanged source height retains an independently configured viewport',
  );
  assert.equal(store.document.design.zoomHint, 1.25);
  store.undo();
  assert.equal(store.document.root.props.Width, '640');
  assert.equal(store.document.design.width, 640);
  assert.equal(store.document.design.height, 500);
  assert.equal(store.document.design.zoomHint, 1.25);
  store.redo();
  assert.equal(store.document.design.width, 800);
  session.dispose();
