...
test('temporary control overrides restore the previous descriptor and notify mounted runtimes', () => {
  const registry = builtins(), previous = registry.get('Button');
  let changes = 0;
  registry.addEventListener('change', () => changes++);
  const dispose = registry.registerControl({type: 'Button', category: 'Application', defaults: {Content: 'Temporary'}});
