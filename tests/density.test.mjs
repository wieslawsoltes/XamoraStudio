...
test('density defaults to Compact, validates preferences, and tolerates unavailable storage', () => {
  ...
  const density = new WorkspaceDensity({ root, storage });
  assert.equal(density.value, 'compact');
  density.set('comfortable');
  assert.equal(root.dataset.density, 'comfortable');
  assert.throws(() => density.set('tiny'));
  assert.equal(density.value, 'comfortable');
  assert.equal(
    new WorkspaceDensity({ root, storage: { getItem: () => '<script>' } }).value,
    'compact',
  );
});
test('density persists and restores without duplicate change notifications', () => {
  ...
  const density = new WorkspaceDensity({ root, storage });
  let changes = 0;
  density.addEventListener('change', () => changes++);
  density.set('standard');
  density.set('standard');
  assert.equal(changes, 1);
  assert.equal(values.get(DENSITY_KEY), 'standard');
  assert.equal(new WorkspaceDensity({ root, storage }).value, 'standard');
  density.set('comfortable', { persist: false });
  assert.equal(values.get(DENSITY_KEY), 'standard');
});
test('switching density retains the live editor buffer, caret, scroll, and document state', () => {
  const dom = installDockDOM();
  document.documentElement = dom.element('html');
  const input = dom.element('textarea');
  input.value = '<Grid>pending';
  ...
  const studio = {
    ...state,
    render() {
      throw Error('Density must not rerender live document controls');
    },
    docking: {
      control: {},
      resize() {
    ...
    direct: {},
    timeline: { applyZoom() {} },
    features: { prototype: { drawConnections() {} } },
  };
  const density = new DensityWorkspace(studio),
    host = dom.element('nav');
  document.body.append(host);
  density.mount(host);
  density.set('comfortable');
  assert.equal(input.value, '<Grid>pending');
  assert.deepEqual([input.selectionStart, input.selectionEnd, input.scrollTop], [2, 7, 96]);
  ...
  assert.equal(density.select.value, 'comfortable');
  assert.equal(resized, 1);
  assert.equal(density.select.children.length, 3);
  density.dispose();
});
test('dock minimum sizes use density chrome metrics recursively while splitters stay five pixels', () => {
  const panels = new Map([
    ...
    split = dockSplit('vertical', dockGroup(['a']), dockGroup(['b']));
